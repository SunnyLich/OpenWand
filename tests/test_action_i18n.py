...
def test_traditional_chinese_catalog_covers_every_action_surface() -> None:
    root = ElementTree.parse("ui/locales/qt/wisp_zh-Hant.ts").getroot()
    translations = {
...
def test_action_text_localizes_templates_without_changing_dynamic_values() -> None:
    translations = {
        ...
        "Wisp couldn't apply the code change: {error}": "FAILED: {error}",
    }
    ...
    assert translate_action_text("Wisp couldn't apply the code change: E_ACCESS", translator=translate) == "FAILED: E_ACCESS"
...
<article class="formatted-reply action-preview">
  <header class="reply-opening">
    ...
    <p class="reply-lede">Review the exact saved-file change before Wisp applies it.</p>
  </header>
...
""".strip()
    translated = {
        ...
        "Review the exact saved-file change before Wisp applies it.": "檢視確切變更。",
        "File": "檔案",
