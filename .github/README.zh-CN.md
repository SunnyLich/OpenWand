...
- **回复呈现控制** — 普通回复无需额外模型调用即可显示清晰的标题、列表、表格、代码、引用和多语言文本；可选的“格式化回复（实验）”附加元件会保留原文，在运行前显示格式化模型，并检查可见的源内容是否完整保留。
- **先预览再执行的应用操作** — 在 Windows 上，可使用三项已验证的 PowerPoint 桌面操作（创建幻灯片、重新设计选中的幻灯片或更新演讲者备注）、用于图表、表格清理、排序和分析的聚焦 LibreOffice Calc 操作，以及聚焦的 VS Code 修复/重构操作。OpenWand 会先预览更改，征得确认后再应用，并在完成后验证结果。
