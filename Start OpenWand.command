#!/usr/bin/env bash
# OpenWand macOS/Linux launcher.
#
# Double-click this on macOS/Linux to start the Python OpenWand app.
...
OPENWAND_APP_LAUNCHED=0

close_macos_terminal_on_exit() {
  if [ "$OS_NAME" != "Darwin" ]; then
    return
  fi
  if [ "$OPENWAND_APP_LAUNCHED" != "1" ]; then
    return
  fi
  if [ "${OPENWAND_KEEP_TERMINAL_ON_EXIT:-}" = "1" ]; then
    return
  fi
  if [ "${OPENWAND_RUNTIME_LOG_MODE:-}" = "debug" ]; then
    return
  fi
  if [ ! -t 0 ]; then
    return
...
STAMP_FILE="$REPO_ROOT/.venv/.openwand-linux-python-deps.stamp"
...
if [ -n "${OPENWAND_LAUNCH_PYTHON:-}" ]; then
  if ! python_matches_want "$OPENWAND_LAUNCH_PYTHON"; then
    echo "ERROR: OPENWAND_LAUNCH_PYTHON is not Python $WANT." >&2
    exit 1
  fi
  if ! ui_deps_ok "$OPENWAND_LAUNCH_PYTHON"; then
    echo "ERROR: OPENWAND_LAUNCH_PYTHON is missing required runtime dependencies." >&2
    exit 1
  fi
  VPY="$OPENWAND_LAUNCH_PYTHON"
else
  setup_venv
fi

export OPENWAND_REPO_ROOT="${OPENWAND_REPO_ROOT:-$REPO_ROOT}"
export PYTHONUNBUFFERED=1

OPENWAND_APP_LAUNCHED=1
