"""Optional rich-reply presentation addon for Wisp."""
...
def get_message_actions(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [{
        ...
        "role": "assistant",
        "presentation": True,
        "auto": _bool_setting("auto_format", False),
    }]
...
def run_message_action(action_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    """Ask Wisp's bounded addon-LLM service for a presentation fragment."""
    ...
    local_model = _local_formatter_model() if route == "ollama-local" else ""
    return {
        "status": "Formatting…",
        "llm": {
            "prompt": prompt,
    ...
    try:
        ...
        if phase == "verify":
            ...
            if not llm_text.strip().upper().startswith("PASS"):
                return {"status": "Meaning check failed. Original kept."}
        ...
        return {"status": f"Formatting failed. Original kept. ({type(exc).__name__})"}
