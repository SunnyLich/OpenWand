/* docs-pages.js — all OpenWand documentation page content */
...
<p>Context is managed dynamically, with you in control: keep it lean so OpenWand captures only what's relevant and token costs stay low, or go deep on long-context queries when the task needs it. Each caller can attach context up front, expose selected sources as model-fetchable tools, or keep them off entirely. Everything is customisable: add your own add-ons, rewrite the prompts, or edit the GUI.</p>
<div class="callout tip"><div class="callout-label">Why it works this way</div><p>This app aims to address 3 main issues with AI tools — and pairs each with what OpenWand does instead.</p><div class="compare"><div class="compare-head"><div class="ch-issue"><span class="compare-dot"></span>The annoyance</div><div class="ch-sol"><span class="compare-dot"></span>With OpenWand</div></div><div class="compare-row"><div class="c-issue">Having to repeatedly type out common prompts</div><div class="c-sol"><strong>Select &amp; hotkey.</strong> Select text with one hand, press a hotkey with the other — saving you seconds every time. STT support means you don't have to type even for custom prompts.</div></div><div class="compare-row"><div class="c-issue">Reading huge bulks of text every time</div><div class="c-sol"><strong>TTS built in.</strong> Listen instead of read, reducing reading fatigue.</div></div><div class="compare-row"><div class="c-issue">Having to switch between the chat window and the app you were using</div><div class="c-sol"><strong>It's an overlay.</strong> OpenWand sits on top, so you never have to switch windows.</div></div></div></div>

<hr />
<h2 id="what-you-get">What you get</h2>
<p>OpenWand lives as a small animated icon in the corner of your screen — always on top, never in your way. Press the hotkey and a quick picker drops in; choose an action or type your own, and OpenWand grabs the right context, streams the reply, and can read it aloud word by word.</p>
...
<table>
  <thead><tr><th>Goal</th><th>How OpenWand addresses it</th></tr></thead>
  <tbody>
...
<div class="demo-grid">
  ...
  <figure class="demo-card">
    ...
    <figcaption>
      ...
      <p>When visual context matters, draw a region with <kbd>Ctrl Alt Q</kbd>. OpenWand sends only that crop to a vision-capable model and keeps the response in the overlay.</p>
    </figcaption>
  </figure>
  ...
  <figure class="demo-card">
    ...
    <figcaption>
      ...
      <p>Delegate a longer workspace job to coordinator, builder, and reviewer roles. The team inspects files, makes a focused change, verifies it, and saves artifacts for review while you keep using OpenWand.</p>
    </figcaption>
  </figure>
</div>`
},
...
<p>For most people, start with a portable package from <a href="https://github.com/SunnyLich/OpenWand/releases" target="_blank">GitHub Releases</a>. Download the package for your OS, unzip it anywhere, and run the included OpenWand app or launcher.</p>
<table>
...
<table>
  ...
  <tbody>
    <tr><td>macOS</td><td><code>Start OpenWand.command</code></td><td><code>requirements/requirements-macos.lock</code> — exact resolved lock</td></tr>
    <tr><td>Linux</td><td><code>Start OpenWand.sh</code></td><td><code>requirements/requirements-linux.lock</code> — exact resolved lock</td></tr>
    <tr><td>Windows</td><td><code>Start OpenWand.bat</code></td><td><code>requirements/requirements-windows.lock</code> — exact resolved lock</td></tr>
  </tbody>
</table>
<div class="callout note"><div class="callout-label">Source checkout only</div><p>OpenWand pins <strong>Python 3.12</strong> in <code>.python-version</code>. The source launchers find a compatible Python 3.12 interpreter automatically — install via <code>pyenv install 3.12</code> on macOS, or from python.org on Windows.</p></div>
...
git clone https://github.com/SunnyLich/OpenWand
cd OpenWand
...
<table>
  ...
  <tbody>
    <tr><td><code>PySide6</code></td><td>All UI — overlay, bubble, settings, chat window. Imported only by the <code>openwand-ui</code> worker.</td></tr>
...
<p>There are two ways to start OpenWand.</p>

<div class="callout tip"><div class="callout-label">Portable version</div><p>Recommended: download a portable package from <a href="https://github.com/SunnyLich/OpenWand/releases" target="_blank">GitHub Releases</a>, unzip it, and run the included OpenWand app or launcher. No repo checkout is needed for that path.</p></div>
<table>
  <thead><tr><th>Platform</th><th>Double-click</th></tr></thead>
  <tbody>
    <tr><td>macOS</td><td><code>OpenWand.app</code></td></tr>
    <tr><td>Linux</td><td><code>OpenWand</code></td></tr>
    <tr><td>Windows</td><td><code>OpenWand.exe</code></td></tr>
  </tbody>
</table>
...
<table>
  <thead><tr><th>Platform</th><th>Double-click</th></tr></thead>
  <tbody>
    <tr><td>macOS</td><td><code>Start OpenWand.command</code></td></tr>
    <tr><td>Linux</td><td><code>Start OpenWand.sh</code></td></tr>
    <tr><td>Windows</td><td><code>Start OpenWand.bat</code></td></tr>
  </tbody>
</table>
<p>For the repo version, the first run installs everything OpenWand needs and then starts the app. Requires Python 3.12 (see <a href="#" onclick="navigate('installation')">Installation</a>).</p>
...
'faq': {
  title: 'Q&A',
  sub: 'Short answers to the questions people ask before using OpenWand.',
...
<table>
  ...
  <tbody>
    <tr><td>Where are chats, memory, and settings stored?</td><td>On your machine. Settings, chats, memory, privacy reports, and local configuration are written to local app data paths, not to a OpenWand-hosted account.</td></tr>
    <tr><td>What is the OS keychain?</td><td>It is the secure password store built into your operating system: Windows Credential Manager on Windows, Keychain on macOS, and Secret Service or KWallet on many Linux desktops. OpenWand uses it for provider keys and OAuth tokens instead of writing them into <code>.env</code> or a plain config file.</td></tr>
    <tr><td>Does OpenWand send everything on my screen?</td><td>No. Context is controlled by caller profile and by the context chips in the intent overlay. OpenWand may inspect available sources locally for availability, token estimates, and redaction counts, but previewing a source does not send it to the model or save it as chat/memory.</td></tr>
...
<table>
  <thead><tr><th>Question</th><th>Answer</th></tr></thead>
  <tbody>
    <tr><td>How can I run it?</td><td>Use the portable package for your OS: Windows <code>.exe</code>, macOS app or launcher, or Linux portable build or launcher. If you are running from the repo, use <code>Start OpenWand.bat</code>, <code>Start OpenWand.command</code>, or <code>Start OpenWand.sh</code>; the first source run installs dependencies, and later runs just launch the app.</td></tr>
    ...
    <tr><td>Can OpenWand update itself?</td><td>Packaged builds can check GitHub Releases from Settings, download the matching artifact, verify its hash, and apply it through a helper that restarts OpenWand. Source checkouts should update with Git.</td></tr>
...
<table>
  <thead><tr><th>Question</th><th>Answer</th></tr></thead>
  <tbody>
    <tr><td>Can I use local models?</td><td>Yes, if they expose an OpenAI-compatible endpoint. Ollama works through its <code>/v1</code> endpoint, and LM Studio / vLLM can be used through the custom endpoint route. OpenWand does not directly speak native, non-OpenAI-compatible local model APIs.</td></tr>
    <tr><td>Can I use more than one provider?</td><td>Yes. Set a primary route and optional fallback routes so OpenWand can switch when a provider is unavailable or limited.</td></tr>
    <tr><td>Why do some models miss tools, images, or long context?</td><td>Provider capabilities differ. OpenWand shows model warnings when the selected route does not support a feature needed by the current request.</td></tr>
...
<table>
  <thead><tr><th>Question</th><th>Answer</th></tr></thead>
  <tbody>
    <tr><td>Is OpenWand free?</td><td>Yes. OpenWand is free and open source. You may still pay for any model provider, TTS provider, or hosted service you choose to connect.</td></tr>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Browser context is empty</td><td>Browser capture is disabled, unsupported, or deferred</td><td>Enable Browser/Web context for the caller. If the chip says deferred, OpenWand may fetch page text only after you send.</td></tr>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Result appears in the bubble but not in the app</td><td>Paste-back disabled or the original target app blocked paste</td><td>Use the rewrite/paste caller, confirm <code>paste_back = True</code>, and test in a plain text editor. OpenWand targets the field captured when the hotkey was pressed.</td></tr>  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>macOS</td><td>Screen, keyboard, or microphone features blocked</td><td>Grant Accessibility, Screen Recording, and Microphone permissions as needed, then restart OpenWand.</td></tr>
...
<div class="callout warn"><div class="callout-label">Status</div><p>OpenWand falls back to CPU automatically when CUDA is not compatible.</p></div>
<table>
  <thead><tr><th>Field</th><th>Details</th></tr></thead>
  <tbody>
    <tr><td>What users see</td><td>The log says Kokoro model failed on cuda: cuDNN version 91900 is not compatible with devices with SM &lt; 7.5, then OpenWand falls back to CPU.</td></tr>
    ...
    <tr><td>Fix status</td><td>OpenWand already falls back safely. We may add a preflight CUDA capability check so unsupported GPUs skip the failed CUDA attempt.</td></tr>
  </tbody>
</table>`
},
...
<p>The overlay is a frameless Qt window owned by the <code>openwand-ui</code> worker, centred on the active screen. It shows a list of single-key intents and auto-closes after <strong>5 seconds</strong> if no key is pressed.</p>
...
<p>One hotkey starts a quick relay: OpenWand gathers the scene, turns your intent into a model request, streams the answer, and keeps useful memory in the background.</p>
<table>
  <thead><tr><th>Beat</th><th>What happens</th><th>Result</th></tr></thead>
  <tbody>
    <tr><td>Summon</td><td>OpenWand captures the active app, selection, clipboard, and enabled context sources.</td><td>The intent picker opens beside the floating icon.</td></tr>
...
'context-capture': {
  ...
  sub: 'How OpenWand reads what\'s on your screen.',
...
<p>The intent overlay and chat window use the same context-policy shape, so the chips you see before sending match the sources OpenWand is allowed to send or expose.</p>
...
<p>When <code>context_documents = True</code>, OpenWand resolves the open document path from the window title and reads its contents. Supported apps include:</p>
<table>
...
<p>When browser context is enabled, OpenWand can fetch the active browser tab, parse HTML with <code>beautifulsoup4</code>, and strip nav/header/footer boilerplate. Private/local URLs are skipped. If the caller uses model-decidable browser context, the page is fetched only when the model calls the context tool.</p>
...
<p>Press <kbd>F7</kbd> to read the current selection aloud with the configured TTS provider. This is separate from model replies: it does not send a query, does not save chat, and is useful when you want OpenWand to read text from the app you are already using.</p>
<p>Auto-spoken replies are opt-in. OpenWand can stream answers into the bubble silently by default, or speak them as they arrive when you enable reply auto-speak in Settings.</p>
...
<p><code>core/tts.py</code> reads replies aloud. OpenWand supports fully local on-device voices, cloud, and OpenAI-compatible speech endpoints — or off entirely:</p>
<table>
  ...
  <tbody>
    ...
    <tr><td><code>gpt_sovits</code></td><td>local</td><td>Local <strong>GPT-SoVITS</strong> voice cloning. You run the GPT-SoVITS server yourself; OpenWand calls its HTTP API at <code>GPT_SOVITS_URL</code> with a short reference clip (<code>GPT_SOVITS_REF_AUDIO_PATH</code>).</td></tr>
    <tr><td><code>none</code></td><td>—</td><td>TTS disabled. Default. LLM reply still streams to the bubble.</td></tr>
  </tbody>
</table>
<p>Long read-aloud selections use one-chunk lookahead: OpenWand plays the current chunk while synthesizing the next, so cancel stays responsive and the full selection is not pre-generated.</p>
...
<p>To mask LLM + TTS latency, OpenWand plays short pre-cached audio clips ("hm…", "let me check…") immediately on hotkey press. Clips are stored as WAV files in <code>assets/filler/</code>.</p>
<table>
...
'live-agents': {
  ...
  sub: 'Run full local agent sessions behind OpenWand, with live progress, approvals, and resumable continuity.',
  toc: ['overview','setup','continuity','live-events','controls','workspace','offline-history'],
  html: `
<h2 id="overview">How the agent modes work</h2>
<p>OpenWand can use more than a model API. It can run a complete local coding-agent harness behind both the overlay and the full chat window. The <strong>ChatGPT</strong> option starts the installed Codex CLI in app-server mode; <strong>Claude Agent</strong> uses the Claude Agent SDK and the same Claude Code CLI authentication. OpenWand supplies the prompt and captured context, then streams the agent's visible output back into the normal OpenWand interface.</p>
<table>
  <thead><tr><th>Run conversations with</th><th>Behavior</th></tr></thead>
  <tbody>
    <tr><td><strong>OpenWand</strong></td><td>Uses the LLM provider and model configured in OpenWand.</td></tr>
...
<div class="callout note"><div class="callout-label">One OpenWand interface</div><p>Hotkey queries and full-chat prompts use the selected engine. OpenWand still captures context, applies privacy controls, displays the stream, and stores a local copy of the conversation.</p></div>

<hr />
<h2 id="setup">Select and sign in</h2>
<p>Open <strong>Settings → App → Run conversations with</strong> and choose OpenWand, ChatGPT, or Claude Agent. When an agent is selected, the same card shows its login status and provides <strong>Sign in</strong>, <strong>Sign out</strong>, and <strong>Refresh</strong> actions. Sign-in opens the provider's terminal and browser flow.</p>
<p>ChatGPT mode requires the Codex CLI. OpenWand gives Codex an isolated local profile, so its login and resumable OpenWand sessions do not appear in your personal Codex history. Claude Agent uses the bundled Claude Agent SDK when available and authenticates through the Claude Code CLI.</p>
...
<table>
  ...
  <tbody>
    <tr><td><strong>OpenWand</strong></td><td>OpenWand sends the complete local OpenWand history on every request. Each request starts without a retained provider continuation link.</td><td>Portable conversations that can change engines without depending on a provider session.</td></tr>
    <tr><td><strong>ChatGPT or Claude Agent</strong></td><td>OpenWand transfers the existing history once, stores the returned provider session ID, and resumes that session for later prompts.</td><td>Long agent conversations that should preserve native provider state efficiently.</td></tr>
  </tbody>
</table>
<p>OpenWand keeps separate OpenWand, ChatGPT/Codex, and Claude conversation namespaces so switching engines does not append a prompt to the wrong provider's history. Changing the project folder also starts a clean provider session.</p>

<hr />
<h2 id="live-events">What appears while the agent works</h2>
<p>Replies stream normally, while visible reasoning summaries, plans, tool starts, command or file progress, status updates, and approval requests appear as progress in the bubble and chat. OpenWand can only show events the provider exposes; private hidden chain-of-thought is never available.</p>
<p>When a command, file change, or other protected tool needs permission, OpenWand presents the action, path, command, or diff it received and sends your decision back to the running agent.</p>

<hr />
<h2 id="controls">Live provider controls</h2>
<p>When ChatGPT or Claude Agent is active, a provider badge appears below the floating OpenWand icon. Click it to change settings for the next turn:</p>
<table>
...
<p>Selected text, ambient app context, attachments, memory, and privacy-filtered content are assembled by OpenWand before the turn. Provider-specific system prompts stay in the native harness instructions instead of being pasted into your message.</p>

<hr />
<h2 id="offline-history">Offline transcript fallback</h2>
<p>Live agent sessions are the normal path. OpenWand can also import selected local ChatGPT/Codex and Claude conversations without contacting either provider, optionally sync changes from the moment automatic sync is enabled, and export an OpenWand chat as a new provider conversation without overwriting existing history.</p>
<div class="callout warn"><div class="callout-label">Local files, separate from live sessions</div><p>Conversation import, forward sync, and export operate on local compatibility files; they do not send a live request or replace the supported Codex app-server and Claude Agent SDK session flow.</p></div>`
},
...
'team-mode': {
  ...
  html: `
<div class="callout note"><div class="callout-label">You configure this team</div><p>Choose <strong>Start Agent Team...</strong> from the tray menu, then set the project folder, goal, agents, models, and permissions. You can close the activity window and keep using OpenWand while the team works.</p></div>
...
<pre><code data-i18n-text-block>1. OpenWand builds a team plan from your goal, context, folder, and permissions
...
'memory': {
  title: 'Memory',
  sub: 'OpenWand remembers things between sessions — all stored locally.',
  toc: ['concept','explicit','auto','retrieval','config'],
  html: `
<h2 id="concept">Concept</h2>
<p>OpenWand persists facts across sessions in a local <strong>JSON memory store</strong> on disk. Everything stays on-device — nothing is sent to an external service for storage or retrieval.</p>
...
<p>When turned on, OpenWand periodically skims recent conversation history and saves anything worth remembering. Off by default — so nothing gets written without you knowing about it.</p>
...
<div id="pillars" class="sec-pillars">
  ...
  <div class="sec-pillar">
    ...
    <p>The full source is public. Nothing about how OpenWand handles your data is hidden behind a binary.</p>
  </div>
</div>
...
<div class="compare">
  <div class="compare-head">
    ...
    <div class="ch-sol"><span class="compare-dot"></span>How OpenWand actually works</div>
  </div>
...
<p>OpenWand offers three mutually exclusive settings under <strong>Settings → App → Privacy mode</strong>:</p>
<table>
...
<p>You don't have to take our word for any of this. OpenWand is fully open source — the redaction patterns, the keychain handling, the network calls, all of it is right there in the repository for you (or your security team) to read and verify.</p>
<div class="callout tip"><div class="callout-label">Verify it yourself</div><p>Read the source on <a href="https://github.com/SunnyLich/OpenWand" target="_blank">GitHub</a> — start with <code>core/context_fetcher.py</code> for redaction and context handling.</p></div>`
},
...
'env-reference': {
  title: '.env reference',
  sub: 'A selected reference for advanced source configuration.',
  toc: ['llm','chat-llm','vision','tts','hotkeys','callers','context','ui','github','memory','system'],
  html: `
<div class="callout note"><div class="callout-label">Source of truth</div><p>This page highlights advanced settings; <code>.env.example</code> is the complete source reference. API keys are <strong>not</strong> stored in <code>.env</code>. Enter them in <strong>Settings → Connections</strong> — they are saved to the OS keychain via <code>keyring</code>.</p></div>
...
You are OpenWand, a concise desktop assistant. Be direct, plainspoken, and useful. Prefer short answers, but expand when the user asks for help, troubleshooting, code, planning, or explanation.
&lt;/role&gt;
...
<p>When <code>CALLER_N_PASTE_BACK=True</code>, OpenWand captures the target field at hotkey time, streams the rewrite visibly in the bubble, and then pastes back into that original field when the answer completes. This is designed for rewrite flows where the selected text should be replaced without losing track of the app you started from.</p>
<hr />
<h2 id="context-toggles">Context toggles</h2>
<p>Each caller has a context grid, not a single three-toggle block. These defaults decide what OpenWand may attach before the model answers, and what the model may fetch on demand during the turn.</p>
<table>
...
<p>OpenWand uses <code>pynput</code> (no admin rights) for caller hotkeys. If a hotkey is already claimed by Windows or another app, OpenWand will not intercept it reliably. Choose combinations that are not globally reserved.</p>
...
<p>Modern extensions live under <code>addons/</code> and declare capabilities in <code>addon.toml</code>. Portable packaged builds create an <code>addons</code> folder beside <code>OpenWand.exe</code> when possible, or use the user-writable add-on folder shown by <strong>Addon Manager → Open addons folder</strong>.</p>
...
<p>The reply bubble is a transparent, always-on-top Qt window owned by the <code>openwand-ui</code> worker. Visual properties can be edited in Settings; source checkouts can also edit the same values in <code>.env</code>:</p>
<table>
  <thead><tr><th>Variable</th><th>Default</th><th>Description</th></tr></thead>
  <tbody>
...
'free-apis': {
  ...
  sub: 'Where to get free or no-cost model access — and how to point OpenWand at it.',
  toc: ['what','hosted','trial','with-openwand','local','notes'],
  html: `
<h2 id="what">Free model access</h2>
<p>OpenWand is free, but it still needs a model provider to answer your queries. You don't have to begin with a paid API key — several providers offer free-tier examples, free monthly credits, or no-cost rate-limited access. This page shows examples of providers you can connect in OpenWand.</p>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><a href="https://github.com/tashfeenahmed/freellmapi" target="_blank" rel="noopener">FreeLLMAPI</a> (self-hosted)</td><td>Open-source MIT gateway you run yourself; pools ~16 providers' free tiers (Google, Groq, Cerebras, Mistral, OpenRouter, GitHub Models, and more) behind one OpenAI-compatible endpoint with automatic failover.</td><td>One token for many free backends; point OpenWand's custom endpoint at your local deployment.</td></tr>
    <tr><td><a href="https://github.com/diegosouzapw/OmniRoute" target="_blank" rel="noopener">OmniRoute</a> (local gateway)</td><td>Open-source router you run locally; aggregates many provider accounts and free tiers behind one OpenAI-compatible endpoint with routing, fallback, and optional compression.</td><td>One local endpoint for many backends; point OpenWand's custom endpoint at OmniRoute and use a model such as <code>auto</code>.</td></tr>
...
<p>Trial credits are useful for evaluating a model before paying, but they are usually spend-limited or time-limited. Use them for comparison runs; build daily OpenWand usage on a permanent free tier, a paid key, or a local model.</p>
<table>
...
<hr />
<h2 id="with-openwand">Using a free source in OpenWand</h2>
<p>OpenWand reaches most of these through its OpenAI-compatible client. Many have a dedicated provider route; account-specific or deployment-specific routes can use independently named custom connections when the provider exposes an OpenAI-compatible URL. Providers without that shape are usually easiest through OpenRouter or another compatible gateway. Add credentials in <strong>Settings → Connections</strong>, where they are stored in the OS keychain, then select the model route in <strong>Settings → LLM</strong>.</p>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Puter.js</td><td>Front-end browser SDK only — it is not a backend API OpenWand can call.</td></tr>
  </tbody>
</table>

<hr />
<h2 id="local">Local, and free for good</h2>
<p>If you run a model on your own machine there are no tokens to bill and nothing leaves the device. <strong>Ollama</strong>, <strong>LM Studio</strong>, and <strong>vLLM</strong> all expose an OpenAI-compatible server that OpenWand talks to through the <code>custom</code> provider.</p>
...
<ul style="padding-left:20px;color:var(--text);font-size:14px;line-height:2">
  ...
  <li>Some free tiers may use your prompts to improve their models — don't send sensitive context to them. OpenWand's <a onclick="navigate('security')">redaction</a> still applies either way.</li>
  ...
  <li>Agent-specific offers such as FreeBuff's free GLM access are not automatically OpenWand API providers. OpenWand needs an API key, a compatible gateway, or a local OpenAI-compatible server.</li>
  ...
  <li>Puter.js is a browser SDK, not a server API, so it can't be set as a OpenWand <code>LLM_PROVIDER</code>.</li>
</ul>`
},
...
<p>Groq exposes an OpenAI-compatible API so OpenWand uses the <code>openai</code> Python package to talk to it. It is a good choice for latency-sensitive hotkey queries thanks to its low time-to-first-token.</p>
...
<p>Enter your Groq API key in <strong>Settings → Connections</strong>. It is stored in the OS keychain.</p>
<hr />
...
<p>Enter your key in <strong>Settings → Connections</strong>.</p>
...
<p>Enter your key in <strong>Settings → Connections</strong>.</p>
<div class="callout note"><div class="callout-label">ChatGPT OAuth is separate</div><p>The OpenAI API route uses an API key. If you want to use a ChatGPT/Codex subscription instead, sign in with OAuth under <strong>Settings → Connections</strong>, then choose the ChatGPT route and model. That route stores tokens in the OS keychain, may require signing in again after restart, is metered against your subscription's agentic allowance, and does not run live context tools the same way API-key providers do.</p></div>
...
<p>Sign in with OAuth under <strong>Settings → Connections</strong>, then choose the ChatGPT route and model. Tokens are stored in the OS keychain.</p>
<div class="callout note"><div class="callout-label">Stable for now, provider-controlled</div><p>This route is stable today, but it depends on OpenAI continuing to allow subscription-backed OAuth access from third-party clients. Provider policy can change later, so keep an API-key, local, or other provider route as a fallback if OpenWand is part of your daily workflow.</p></div>
...
<hr />
<h2 id="models">Models</h2>
<p>Model availability depends on your subscription and what the OAuth route exposes to OpenWand. Start with the default shown in Settings, then adjust only if the selected model is available on your account.</p>`
},
...
<p>Enter your Google AI Studio API key in <strong>Settings → Connections</strong>.</p>
...
<p>OpenWand uses the <code>openai</code> Python package for all OpenAI-compatible endpoints. The following providers work by setting the right <code>LLM_PROVIDER</code> value and adding the API key in Settings:</p>
<table>
...
<p>Enter the corresponding API key in <strong>Settings → Connections</strong>.</p>`
},

'provider-custom': {
  title: 'Custom endpoint',
  sub: 'Any OpenAI-compatible server — local Ollama, LM Studio, vLLM, etc.',
  toc: ['setup','legacy-env','ollama','notes'],
  html: `
<h2 id="setup">Add a named custom connection</h2>
<p>Open <strong>Settings → Connections</strong>, add a custom connection, and give it a recognizable name, base URL, and API key. Each connection receives a stable <code>custom@&lt;id&gt;</code> route, so you can keep multiple local servers, gateways, accounts, or deployments and choose the right one under <strong>Settings → LLM</strong>.</p>

<hr />
<h2 id="legacy-env">Legacy environment setup</h2>
<p>The singleton environment variables remain available for source builds and backward compatibility. Prefer named connections for normal setup.</p>
<pre><span class="pre-lang">env</span><code><span class="c-key">LLM_PROVIDER</span>=<span class="c-val">custom</span>
...
<hr />
<h2 id="notes">Notes</h2>
<div class="callout note"><div class="callout-label">Antivirus</div><p>Some antivirus products flag <code>keyboard</code> hooks. You may need to add the app directory or <code>OpenWand.exe</code> to your AV exclusion list.</p></div>
...
'platform-macos': {
  ...
  html: `
<h2 id="status">Status</h2>
<p>OpenWand runs natively on macOS 13 (Ventura) and later, on both Apple Silicon and Intel Macs. The overlay, voice, context capture, and memory are all supported.</p>
<div class="callout note"><div class="callout-label">macOS packaged build status</div><p>The packaged macOS build was last live-tested quite a while ago, so it may be buggier than the Windows build or the repo launcher path. If it gives you trouble, please try the repo version with <code>Start OpenWand.command</code>; it is the best-supported macOS path right now. Renting Apple hardware for fresh testing costs money, so if you would like to support more macOS verification, you can donate at <a href="https://buymeacoffee.com/sunnylich" target="_blank">Buy Me a Coffee</a>. No pressure either way: clear bug reports with logs are also very helpful.</p></div>
<table>
...
<p>macOS gates input and screen APIs behind the privacy system (TCC). On first run, grant OpenWand the following under <strong>System Settings → Privacy &amp; Security</strong>:</p>
<ul style="padding-left:20px;color:var(--text);font-size:14px;line-height:2">
  <li><strong>Accessibility</strong> — required for global hotkeys and reading the focused element</li>
  <li><strong>Input Monitoring</strong> — required for the global hotkey listener (a purpose-built PyObjC/Carbon backend in <code>openwand-native</code>)</li>
  <li><strong>Screen Recording</strong> — required only for the snip overlay</li>
</ul>
<div class="callout warn"><div class="callout-label">Restart after granting</div><p>macOS only applies new Accessibility / Input Monitoring grants to a process after it is relaunched. Quit and reopen OpenWand once permissions are checked.</p></div>
...
<p>For a session that keeps full runtime logs, start OpenWand with <code>Start OpenWand Debug.command</code> instead of the normal launcher.</p>`
},
...
<hr />
<h2 id="notes">Notes</h2>
<div class="callout note"><div class="callout-label">Wayland in progress</div><p>OpenWand is best supported on X11 sessions today. We are currently working on Linux Wayland support; native hotkey, clipboard, and screen capture behavior still depends on the desktop environment.</p></div>
...
'addons': {
  title: 'Add-ons',
  sub: 'Extend OpenWand with query hooks, tray actions, settings, and model-callable tools — each in its own process.',
  toc: ['concept','ideas','isolation','layout','manifest','permissions','hooks','events','dependencies','enabling','mcp-client','mcp-server'],
  html: `
<h2 id="concept">Concept</h2>
<p>Add-ons are the supported way to extend OpenWand. An add-on can observe or modify query context, observe responses, contribute tray actions, expose settings, register model-callable tools, and declare its own intents and hotkeys.</p>
...
<p>Each enabled add-on runs in its <strong>own Python host process</strong> — one process per add-on. A crash, import failure, or slow hook is isolated from the brain worker and from every other add-on. OpenWand talks to each host over a small newline-delimited JSON IPC protocol.</p>
...
<hr />
<h2 id="permissions">Permissions</h2>
<p>Capabilities are opt-in — <strong>missing permissions are denied</strong>. An add-on without <code>tools = true</code> can't register tools; one without <code>ui = ["tray"]</code> can't add tray actions. LLM actions require <code>llm = true</code> and are capped by OpenWand before any provider credentials are used.</p>
<table>
...
<p><code>[dependencies]</code> is optional. Add-ons without it run from OpenWand's own Python runtime. Add-ons that declare packages get a dedicated virtual environment under <code>addon_envs/&lt;id&gt;/</code>; the Addon Manager shows the required packages and offers an Install/Repair action.</p>
<div class="callout note"><div class="callout-label">Approval per dependency hash</div><p>OpenWand records approval for the exact dependency set, so an update that changes packages must be approved again before it runs. <code>uv</code> is used when available, falling back to <code>python -m venv</code> in source checkouts.</p></div>
...
<p>Distribution is supported with <code>.zip</code> or <code>.openwand</code> archives containing one add-on folder; the Addon Manager can also install from an unpacked folder.</p>

<hr />
<h2 id="mcp-client">MCP client: use external servers inside OpenWand</h2>
<p>OpenWand ships with an <strong>MCP bridge</strong> add-on (<code>addons/mcp_bridge</code>) that acts as a <a href="https://modelcontextprotocol.io" target="_blank" rel="noopener">Model Context Protocol</a> client. List servers in its <code>servers.json</code>, and OpenWand connects to them and exposes their toolkit to its model as OpenWand tools. This lets the overlay use external MCP capabilities without leaving your desktop workflow. A dependency-free <code>example_server.py</code> is included for trying the bridge.</p>

<hr />
<h2 id="mcp-server">MCP server: give AI clients OpenWand desktop context</h2>
<p>The same bundled add-on also includes the <strong>OpenWand Context Server</strong> (<code>context_server.py</code>): a local MCP stdio server that trusted external clients can launch to read desktop context through OpenWand's capture machinery. It does not require the OpenWand app to stay open.</p>
<ul style="padding-left:20px;color:var(--text);font-size:14px;line-height:2">
...
<div class="callout tip"><div class="callout-label">Add a fallback</div><p>Define at least one <code>LLM_FALLBACKS</code> route so a single provider outage or rate limit doesn't break your hotkeys — OpenWand tries each route in order.</p></div>`
},

'building-exe': {
  title: 'Building a portable version',
  sub: 'Package OpenWand as a portable app and publish release artifacts.',
...
<pre><code>dist/OpenWand/</code></pre>
<p>Run the packaged app from inside that folder:</p>
<pre><code>dist/OpenWand/OpenWand.exe</code></pre>
...
<p>Tagged releases are built by <code>.github/workflows/build.yml</code>. The workflow uploads one artifact per supported platform plus <code>openwand-release-manifest.json</code>, which powers the Settings update button.</p>
<table>
  <thead><tr><th>Platform</th><th>Release artifact</th></tr></thead>
  <tbody>
    <tr><td>Windows</td><td><code>OpenWand-&lt;tag&gt;-windows-x64.zip</code></td></tr>
    <tr><td>macOS</td><td><code>OpenWand-&lt;tag&gt;-macos-&lt;arch&gt;.zip</code></td></tr>
    <tr><td>Linux</td><td><code>OpenWand-&lt;tag&gt;-linux-x64.tar.gz</code></td></tr>
  </tbody>
</table>
<p>The manifest lets packaged OpenWand builds find the newest compatible asset, verify its SHA256 hash after download, and apply it through a helper process that waits for OpenWand to exit, replaces the packaged app folder, and restarts OpenWand.</p>
...
<ul style="padding-left:20px;color:var(--text);font-size:14px;line-height:2">
  ...
  <li>Keep the contents of <code>dist/OpenWand/</code> together when moving the portable build to another folder or machine.</li>
