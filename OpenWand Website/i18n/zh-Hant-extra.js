...
Object.assign(I18N.reg['zh-Hant'].tr, {
  ...
  'Using a free source in OpenWand': '在 OpenWand 中使用免費來源',
  ...
  "OpenWand is free, but it still needs a model provider to answer your queries. You don't have to begin with a paid API key — several providers offer a genuinely free tier, free monthly credits, or no-cost rate-limited access. This page rounds up the current options and shows how to connect each one to OpenWand.":
    "OpenWand 是免費的，但它仍然需要一個模型供應商來回答你的查詢。你不必一開始就使用付費 API 金鑰——多個供應商提供真正的免費額度、每月贈送額度或限速的零成本存取。本頁彙整了目前的選項，並說明如何將每一個連接到 OpenWand。",
  ...
  "OpenWand reaches most of these through its OpenAI-compatible client. A few have a dedicated LLM_PROVIDER value; everything else works through the custom endpoint by pointing CUSTOM_BASE_URL at the provider's OpenAI-compatible URL. Add the key itself in Settings → LLM, where it is stored in the OS keychain.":
    "OpenWand 透過其相容 OpenAI 的用戶端存取其中大多數。少數供應商有專用的 <code>LLM_PROVIDER</code> 值；其餘的都透過 <code>custom</code> 端點運作，只需將 <code>CUSTOM_BASE_URL</code> 指向供應商相容 OpenAI 的 URL。金鑰本身使在 <strong>設定 → LLM</strong> 中填寫，它會儲存到作業系統金鑰鏈中。",
  'If you run a model on your own machine there are no tokens to bill and nothing leaves the device. Ollama, LM Studio, and vLLM all expose an OpenAI-compatible server that OpenWand talks to through the custom provider.':
    "如果你在自己的機器上執行模型，就沒有 token 需要計費，也沒有任何資料離開裝置。<strong>Ollama</strong>、<strong>LM Studio</strong> 與 <strong>vLLM</strong> 都會公開一個相容 OpenAI 的伺服器，OpenWand 透過 <code>custom</code> 供應商與之通訊。",
  ...
  'Front-end browser SDK only — it is not a backend API OpenWand can call.':
    "僅為前端瀏覽器 SDK——它不是 OpenWand 可以呼叫的後端 API。",
  ...
  "Some free tiers may use your prompts to improve their models — don't send sensitive context to them. OpenWand's redaction still applies either way.":
    "有些免費額度可能會用你的提示詞來改進其模型——不要向它們傳送敏感情境。無論如何，OpenWand 的<a onclick=\"navigate('security')\">脫敏</a>依然生效。",
  ...
  "Puter.js is a browser SDK, not a server API, so it can't be set as a OpenWand LLM_PROVIDER.":
    "Puter.js 是瀏覽器 SDK，而非伺服器 API，因此無法設為 OpenWand 的 <code>LLM_PROVIDER</code>。",
  ...
  'OpenWand lives as a small animated icon in the corner of your screen — always on top, never in your way. Press the hotkey and a quick picker drops in; choose an action or type your own, and OpenWand grabs the right context, streams the reply, and can read it aloud word by word.':
    'OpenWand 以一個小巧的動態圖示停在螢幕角落——始終置頂，卻從不礙事。按下快速鍵，快捷選擇器隨即出現；選擇一個動作或自行輸入，OpenWand 便會擷取合適的情境，在約一秒半內作答，並逐字朗讀回覆。',
  'Any app': '任何應用程式',
  ...
  'OpenWand listens for your custom hotkey across apps, opens with minimal prompt delay, and sends the selected context without a mouse or window switch.':
    'OpenWand 會在所有應用程式中監聽你的自訂快速鍵，以極低延遲開啟提問介面，並送出選取的情境；無需滑鼠，也無需切換視窗。',
  ...
  'When CALLER_N_PASTE_BACK=True, OpenWand pastes the reply straight into whichever input had focus before the overlay opened — replacing the selected text.':
    '當 <code>CALLER_N_PASTE_BACK=True</code> 時，OpenWand 會將回覆直接貼到覆蓋介面開啟前擁有焦點的輸入欄位中——取代選取的文字。',
  ...
  'OpenWand uses pynput (no admin rights) for caller hotkeys. If a hotkey is already claimed by Windows or another app, OpenWand will not intercept it reliably. Choose combinations that are not globally reserved.':
    'OpenWand 使用 <code>pynput</code>（無需管理員權限）處理呼叫器快速鍵。如果某個快速鍵已被 Windows 或其他應用程式占用，OpenWand 將無法可靠地攔截它。使選擇未被全域保留的組合。',
  ...
  'The reply bubble is a transparent, always-on-top Qt window owned by the openwand-ui worker. Visual properties can be edited in Settings; source checkouts can also edit the same values in .env:':
    '回覆泡泡是一個透明、始終置頂的 Qt 視窗，由 <code>openwand-ui</code> 工作行程擁有。視覺屬性可以在設定中編輯；原始碼版本也可以在 <code>.env</code> 中編輯相同的值：',
  ...
  'Groq exposes an OpenAI-compatible API so OpenWand uses the openai Python package to talk to it. It is a good choice for latency-sensitive hotkey queries thanks to its low time-to-first-token.':
    'Groq 提供與 OpenAI 相容的 API，因此 OpenWand 使用 <code>openai</code> Python 套件與之通訊。憑藉極低的首字延遲，它是對延遲敏感的快速鍵查詢的理想選擇。',
  ...
  'This route is stable today, but it depends on OpenAI continuing to allow subscription-backed OAuth access from third-party clients. Provider policy can change later, so keep an API-key, local, or other provider route as a fallback if OpenWand is part of your daily workflow.':
    '這條路線目前穩定，但它依賴 OpenAI 持續允許第三方用戶端使用訂閱支援的 OAuth 存取。供應商政策之後可能變更，所以如果 OpenWand 是你的日常工作流程，請保留 API 金鑰、本機模型或其他供應商作為備援路線。',
  ...
  'Model availability depends on your subscription and what the OAuth route exposes to OpenWand. Start with the default shown in Settings, then adjust only if the selected model is available on your account.':
    '模型可用性取決於你的訂閱以及 OAuth 路線向 OpenWand 暴露的內容。先使用設定中顯示的預設值；只有當所選模型在你的帳戶中可用時再調整。',
  ...
  'OpenWand uses the openai Python package for all OpenAI-compatible endpoints. The following providers work by setting the right LLM_PROVIDER value and adding the API key in Settings:':
    'OpenWand 對所有相容 OpenAI 的端點都使用 <code>openai</code> Python 套件。以下供應商只需設定正確的 <code>LLM_PROVIDER</code> 值並在設定中加入 API 金鑰即可使用：',
  ...
  'Some antivirus products flag keyboard hooks. You may need to add the app directory or OpenWand.exe to your AV exclusion list.':
    '某些防毒軟體會標記 <code>keyboard</code> 掛鉤。你可能需要將應用程式目錄或 <code>OpenWand.exe</code> 加入防毒軟體的排除清單中。',
  ...
  'OpenWand runs natively on macOS 13 (Ventura) and later, on both Apple Silicon and Intel Macs. The overlay, voice, context capture, and memory are all supported.':
    'OpenWand 在 macOS 13（Ventura）及更新版本上原生執行，支援 Apple Silicon 與 Intel Mac。覆蓋介面、語音、情境擷取與記憶均受支援。',
  ...
  'The packaged macOS build was last live-tested quite a while ago, so it may be buggier than the Windows build or the repo launcher path. If it gives you trouble, please try the repo version with Start OpenWand.command; it is the best-supported macOS path right now. Renting Apple hardware for fresh testing costs money, so if you would like to support more macOS verification, you can donate at Buy Me a Coffee. No pressure either way: clear bug reports with logs are also very helpful.':
    'macOS 打包版距離上次真機實測已經有一段時間，因此可能比 Windows 版本或儲存庫啟動器路徑更容易出問題。如果遇到問題，使嘗試使用 <code>Start OpenWand.command</code> 執行儲存庫版本；這是目前支援最好的 macOS 路徑。租用 Apple 硬體進行新的測試需要費用，所以如果你想支持更多 macOS 驗證，可以在 <a href="https://buymeacoffee.com/sunnylich" target="_blank">Buy Me a Coffee</a> 捐助。當然完全沒有壓力：附帶日誌的清楚 bug 回報也非常有幫助。',
  'Area': '方面',
  'Full support': '完整支援',
  ...
  'macOS gates input and screen APIs behind the privacy system (TCC). On first run, grant OpenWand the following under System Settings → Privacy & Security:':
    'macOS 透過隱私系統（TCC）對輸入與螢幕 API 設限。首次執行時，使在<strong>系統設定 → 隱私權與安全性</strong>中授予 OpenWand 以下權限：',
  ...
  'Input Monitoring — required for the global hotkey listener (a purpose-built PyObjC/Carbon backend in openwand-native)':
    '<strong>輸入監控</strong>——全域快速鍵監聽器所必需（<code>openwand-native</code> 中專門打造的 PyObjC/Carbon 後端）',
  ...
  'macOS only applies new Accessibility / Input Monitoring grants to a process after it is relaunched. Quit and reopen OpenWand once permissions are checked.':
    'macOS 只有在行程重新啟動後才會對其套用新的輔助使用 / 輸入監控授權。勾選權限後，使結束並重新開啟 OpenWand。',
  ...
  'For a session that keeps full runtime logs, start OpenWand with Start OpenWand Debug.command instead of the normal launcher.':
    '若要讓工作階段保留完整的執行記錄，請使用 <code>Start OpenWand Debug.command</code> 而非一般啟動器來啟動 OpenWand。',
  ...
  'OpenWand is best supported on X11 sessions today. We are currently working on Linux Wayland support; native hotkey, clipboard, and screen capture behavior still depends on the desktop environment.':
    'OpenWand 目前在 X11 工作階段上的支援最好。我們正在推進 Linux Wayland 支援；原生快捷鍵、剪貼簿和螢幕擷取行為仍取決於桌面環境。',
  ...
  'Add-ons are the supported way to extend OpenWand. An add-on can observe or modify query context, observe responses, contribute tray actions, expose settings, register model-callable tools, and declare its own intents and hotkeys.':
    '附加元件是擴充 OpenWand 的受支援方式。附加元件可以觀察或修改查詢情境、觀察回應、貢獻工具列動作、公開設定、註冊模型可呼叫的工具，並宣告自己的意圖與快速鍵。',
  ...
  'Each enabled add-on runs in its own Python host process — one process per add-on. A crash, import failure, or slow hook is isolated from the brain worker and from every other add-on. OpenWand talks to each host over a small newline-delimited JSON IPC protocol.':
    '每個啟用的附加元件都在<strong>自己的 Python 主機行程</strong>中執行——每個附加元件一個行程。當機、匯入失敗或緩慢的掛鉤都會與「大腦」工作行程以及所有其他附加元件隔離。OpenWand 透過一個以換行分隔的小型 JSON IPC 協定與每個主機通訊。',
  'Layout': '配置',
  ...
  'Capabilities are opt-in — missing permissions are denied. An add-on without tools = true can\'t register tools; one without ui = ["tray"] can\'t add tray actions. LLM actions require llm = true and are capped by OpenWand before any provider credentials are used.':
    '能力是需主動啟用的——<strong>缺少的權限會被拒絕</strong>。沒有 <code>tools = true</code> 的附加元件無法註冊工具；沒有 <code>ui = [\"tray\"]</code> 的則無法新增工具列動作。LLM 動作需要 <code>llm = true</code>，並在使用任何供應商憑證之前由 OpenWand 設限。',
  ...
  '[dependencies] is optional. Add-ons without it run from OpenWand\'s own Python runtime. Add-ons that declare packages get a dedicated virtual environment under addon_envs/<id>/; the Addon Manager shows the required packages and offers an Install/Repair action.':
    '<code>[dependencies]</code> 是選用的。沒有它的附加元件將從 OpenWand 自己的 Python 執行階段執行。宣告了套件的附加元件會在 <code>addon_envs/&lt;id&gt;/</code> 下取得專用的虛擬環境；附加元件管理員會顯示所需套件並提供「安裝/修復」動作。',
  ...
  'OpenWand records approval for the exact dependency set, so an update that changes packages must be approved again before it runs. uv is used when available, falling back to python -m venv in source checkouts.':
    'OpenWand 會記錄對確切相依集合的核准，因此變更套件的更新在執行前必須再次獲得核准。可用時使用 <code>uv</code>，在原始碼簽出中回退到 <code>python -m venv</code>。',
  ...
  'Distribution is supported with .zip or .openwand archives containing one add-on folder; the Addon Manager can also install from an unpacked folder.':
    '支援以包含單一附加元件資料夾的 <code>.zip</code> 或 <code>.openwand</code> 封存進行散布；附加元件管理員也可從解壓縮後的資料夾安裝。',
  ...
  'Script tools in tools/installed/ still load, but the supported way to extend OpenWand is now Add-ons — they run in isolated processes and do far more than register a tool.':
    '<code>tools/installed/</code> 中的指令碼工具仍會載入，但現在擴充 OpenWand 的受支援方式是<a onclick="navigate(\'addons\')">附加元件</a>——它們在隔離的行程中執行，所做的遠不止註冊一個工具。',
  ...
  'Tool plugins run in the same process as OpenWand with full OS access. Only install plugins you trust.':
    '工具外掛與 OpenWand 在同一行程中執行，擁有完整的作業系統存取權限。使只安裝你信任的外掛。',
  ...
  "Define at least one LLM_FALLBACKS route so a single provider outage or rate limit doesn't break your hotkeys — OpenWand tries each route in order.":
    '至少定義一條 <code>LLM_FALLBACKS</code> 路線，這樣單一供應商的故障或速率限制就不會讓你的快速鍵失靈——OpenWand 會依序嘗試每條路線。',
  ...
  'Keep the contents of dist/OpenWand/ together when moving the portable build to another folder or machine.':
    '將可攜版建置移到其他資料夾或機器時，使保持 <code>dist/OpenWand/</code> 內的內容在一起。',
  ...
  'On your machine. Settings, chats, memory, privacy reports, and local configuration are written to local app data paths, not to a OpenWand-hosted account.':
    '在你的機器上。設定、聊天、記憶、隱私報告和本機設定都會寫入本機應用程式資料路徑，而不是寫入由 OpenWand 託管的帳號。',
  ...
  'It is the secure password store built into your operating system: Windows Credential Manager on Windows, Keychain on macOS, and Secret Service or KWallet on many Linux desktops. OpenWand uses it for provider keys and OAuth tokens instead of writing them into .env or a plain config file.':
    '它是內建在作業系統中的安全密碼儲存區：Windows 上是認證管理員，macOS 上是鑰匙圈，許多 Linux 桌面上則是 Secret Service 或 KWallet。OpenWand 用它儲存供應商金鑰和 OAuth token，而不是寫入 <code>.env</code> 或純文字設定檔。',
  'Does OpenWand send everything on my screen?': 'OpenWand 會傳送我螢幕上的所有內容嗎？',
  'No. Context is controlled by caller profile and by the context chips in the intent overlay. OpenWand may inspect available sources locally for availability, token estimates, and redaction counts, but previewing a source does not send it to the model or save it as chat/memory.':
    '不會。情境由呼叫器設定檔和意圖覆蓋介面中的情境標籤控制。OpenWand 可能會在本機檢查可用來源，用於顯示可用性、token 估算和遮蔽計數，但預覽某個來源不會把它傳送給模型，也不會儲存為聊天或記憶。',
  ...
  'Use the portable package for your OS: Windows .exe, macOS app or launcher, or Linux portable build or launcher. If you are running from the repo, use Start OpenWand.bat, Start OpenWand.command, or Start OpenWand.sh; the first source run installs dependencies, and later runs just launch the app.':
    '請使用適合你作業系統的可攜套件：Windows 的 <code>.exe</code>、macOS 應用程式或啟動器，或 Linux 可攜版或啟動器。如果你從原始碼版本執行，請使用 <code>Start OpenWand.bat</code>、<code>Start OpenWand.command</code> 或 <code>Start OpenWand.sh</code>；第一次會安裝相依套件，之後只會啟動應用程式。',
  ...
  'Yes, if they expose an OpenAI-compatible endpoint. Ollama works through its /v1 endpoint, and LM Studio / vLLM can be used through the custom endpoint route. OpenWand does not directly speak native, non-OpenAI-compatible local model APIs.':
    '可以，只要它們提供 OpenAI 相容端點即可。Ollama 透過它的 <code>/v1</code> 端點運作，LM Studio / vLLM 則可透過自訂端點路由使用。OpenWand 目前不會直接呼叫原生、非 OpenAI 相容的本機模型 API。',
  ...
  'Yes. Set a primary route and optional fallback routes so OpenWand can switch when a provider is unavailable or limited.':
    '可以。設定一條主要路線和選用的備援路線，這樣當某個供應商無法使用或受限時，OpenWand 可以切換。',
  ...
  'Provider capabilities differ. OpenWand shows model warnings when the selected route does not support a feature needed by the current request.':
    '不同供應商的能力不同。當所選路線不支援目前使求所需功能時，OpenWand 會顯示模型警告。',
  ...
  'Is OpenWand free?': 'OpenWand 是免費的嗎？',
  'Yes. OpenWand is free and open source. You may still pay for any model provider, TTS provider, or hosted service you choose to connect.':
    '是。OpenWand 免費且開源。不過你選擇連接的模型供應商、TTS 供應商或託管服務仍可能需要付費。',
  ...
  /* Common issues */
  'Known issues': '已知問題',
  'Kokoro TTS falls back to CPU on older NVIDIA GPUs': 'Kokoro TTS 在較舊的 NVIDIA GPU 上回退到 CPU',
  'OpenWand falls back to CPU automatically when CUDA is not compatible.':
    '當 CUDA 不相容時，OpenWand 會自動回退到 CPU。',
  'Details': '詳細資訊',
  ...
  'The log says Kokoro model failed on cuda: cuDNN version 91900 is not compatible with devices with SM < 7.5, then OpenWand falls back to CPU.':
    '日誌會顯示 Kokoro 模型在 <code>cuda</code> 上失敗：cuDNN 版本 91900 與 SM 低於 7.5 的裝置不相容，隨後 OpenWand 會回退到 CPU。',
  'Affected systems': '受影響系統',
  'NVIDIA GPUs with compute capability below 7.5 when paired with a PyTorch/cuDNN build that requires newer CUDA hardware.':
    '計算能力低於 7.5 的 NVIDIA GPU，並且搭配了需要更新 CUDA 硬體的 PyTorch/cuDNN 建置。',
  'Impact': '影響',
  'Local Kokoro TTS still works, but speech synthesis uses CPU and may be slower.':
    '本機 Kokoro TTS 仍可運作，但語音合成會使用 CPU，速度可能較慢。',
  'Fix status': '修復狀態',
  'OpenWand already falls back safely. We may add a preflight CUDA capability check so unsupported GPUs skip the failed CUDA attempt.':
    'OpenWand 已經會安全回退。之後可能會加入 CUDA 能力預先檢查，讓不支援的 GPU 跳過注定失敗的 CUDA 嘗試。',
  'Start here': '從這裡開始',
  ...
  'Install Python 3.12, then rerun Start OpenWand.command. macOS installs from requirements/requirements-macos.lock.':
    '安裝 Python <code>3.12</code>，然後重新執行 <code>Start OpenWand.command</code>。macOS 會從 <code>requirements/requirements-macos.lock</code> 安裝。',
  ...
  'Enable Browser/Web context for the caller. If the chip says deferred, OpenWand may fetch page text only after you send.':
    '為呼叫器啟用瀏覽器/網頁情境。如果標籤顯示延後，OpenWand 可能只會在傳送後擷取頁面文字。',
  ...
  'Grant Accessibility, Screen Recording, and Microphone permissions as needed, then restart OpenWand.':
    '根據需要授予輔助使用、螢幕錄製和麥克風權限，然後重新啟動 OpenWand。',
  'Linux': 'Linux',
...
Object.assign(I18N.reg['zh-Hant'].tr, {
  ...
  "OpenWand offers three mutually exclusive settings under Settings → App → Privacy mode:": "OpenWand 在<strong>設定 → 應用程式 → 隱私模式</strong>下提供三個互斥選項：",
  "Mode": "模式",
  "What it does": "功能",
  ...
  "OpenWand loads the advanced model into memory and warms it in the background when OpenWand starts or after you enable Advanced mode. Warm-up may take tens of seconds on a CPU. If you send a request before it finishes, that request waits; later scans reuse the loaded model and are faster.": "OpenWand 啟動時或您啟用進階模式後，會在背景將進階模型載入記憶體並進行預熱。在 CPU 上，預熱可能需要數十秒。如果您在預熱完成前傳送請求，該請求會等待；後續掃描會重複使用已載入的模型，因此速度更快。",
  "OpenWand merges the AI model's findings with the built-in detector, replaces sensitive spans with stable session placeholders such as [PERSON_1], and can show a review before the request is sent. It then checks the redacted text again. If the model is unavailable, detection fails, or sensitive text remains, OpenWand blocks the cloud send.": "OpenWand 會合併 AI 模型與內建偵測器的結果，將敏感片段替換為 <code>[PERSON_1]</code> 等穩定的工作階段預留位置，並可在請求送出前顯示審查畫面。隨後，它會再次檢查去識別化文字。如果模型無法使用、偵測失敗或仍有敏感文字殘留，OpenWand 會阻止傳送至雲端。",
  "Prompt injection protection": "提示注入防護",
  "This checks captured text for simple attempts to override the model's instructions. You can enable detection and choose whether OpenWand warns before sending, so you can continue or cancel.": "此功能會檢查擷取的文字，尋找試圖覆寫模型指令的簡單表達。您可以啟用偵測，並選擇是否在傳送前顯示警告，以便繼續或取消請求。",
...
Object.assign(I18N.reg['zh-Hant'].meta, {
  'technical-demos': {
    title: '技術示範',
    sub: 'OpenWand 在提示詞、應用程式情境、視覺、改寫、操作和代理團隊任務中的真實執行。',
  },
});
...
Object.assign(I18N.reg['zh-Hant'].tr, {
  'These clips show OpenWand staying in the current app, collecting the right context, acting on focused work, and handing longer jobs to an Agent Team.':
    '這些片段展示 OpenWand 如何停留在目前應用程式中、收集合適的情境、處理聚焦的工作，並將更長的任務交給代理團隊。',
  'Prompt from anywhere': '隨處發起提示',
  'Select content in any app, invoke OpenWand, and run a saved or custom prompt without leaving what you are working on.':
    '在任意應用程式中選取內容，叫出 OpenWand，然後執行已儲存或自訂的提示詞，無需離開目前工作。',
  'Cross-app context': '跨應用程式情境',
  'OpenWand can combine the active selection with enabled browser and app context, giving the model the material it needs without manual copy-paste.':
    'OpenWand 可以將目前選取內容與已啟用的瀏覽器及應用程式情境結合，為模型提供所需材料，無需手動複製貼上。',
  'Rewrite in place': '原位改寫',
  'Rewrite only the selected text, review the proposed wording, and paste the accepted result back into the field that was active when you invoked OpenWand.':
    '僅改寫選取的文字，審閱建議措辭，然後將接受的結果貼回叫出 OpenWand 時處於作用中的欄位。',
  'App-aware action': '應用程式感知操作',
  'Use focused application context to analyze or act on the current work, with a clear result and confirmation when no document cells were changed.':
    '使用聚焦的應用程式情境分析目前工作或執行操作，並在未變更任何儲存格時提供明確結果與確認。',
  'OpenWand prompt demo': 'OpenWand 提示詞示範',
  'OpenWand cross-app context demo': 'OpenWand 跨應用程式情境示範',
  'OpenWand rewrite demo': 'OpenWand 改寫示範',
  'OpenWand app-aware action demo': 'OpenWand 應用程式感知操作示範',
  'OpenWand Agent Team task demo': 'OpenWand 代理團隊任務示範',
  'These clips show OpenWand doing the practical work behind the docs: staying in the current app, collecting the right context, and handing longer tasks to the experimental agent framework.':
    '這些片段展示了文件背後 OpenWand 真正做事的樣子：停留在目前應用程式中，收集合適的情境，並把更長的任務交給實驗性的代理框架。',
  'Overlay query': '覆蓋層查詢',
  'The core OpenWand loop: press the hotkey, choose an intent, send selected or enabled context, and read the streamed answer without leaving the active app.':
    'OpenWand 的核心流程：按下快速鍵，選擇意圖，傳送選取或已啟用的情境，然後在不離開目前應用程式的情況下閱讀串流回答。',
  'Vision snip': '視覺截圖',
  'When visual context matters, draw a region with Ctrl Alt Q. OpenWand sends only that crop to a vision-capable model and keeps the response in the overlay.':
    '當視覺情境很重要時，用 <kbd>Ctrl Alt Q</kbd> 框選一個區域。OpenWand 只會把這塊截圖傳送給支援視覺的模型，並把回答保留在覆蓋層中。',
  ...
  'OpenWand can gather useful app context without taking a screenshot, so the model knows what you are working on. Then the rewrite hotkey rewrites only the selected text and targets paste-back at the original field captured when you pressed the hotkey.':
    '這個示範展示兩個不同功能。首先，OpenWand 可以在不截圖的情況下收集有用的應用程式情境，讓模型知道你正在處理什麼。然後，改寫快速鍵只重寫選取的文字，並把貼回目標指向按下快速鍵時擷取的原始欄位。',
  ...
  'OpenWand hotkey overlay query demo': 'OpenWand 快速鍵覆蓋層查詢示範',
  'OpenWand screen snip demo': 'OpenWand 螢幕截圖示範',
  'OpenWand context-aware rewrite demo': 'OpenWand 情境感知重寫示範',
  'OpenWand multi-agent task demo': 'OpenWand 多代理任務示範',
});
...
Object.assign(I18N.reg['zh-Hant'].tr, {
  ...
  "Each caller has a context grid, not a single three-toggle block. These defaults decide what OpenWand may attach before the model answers, and what the model may fetch on demand during the turn.": "每個呼叫器都有一個情境網格，而不是單一的三開關區塊。這些預設值決定了 OpenWand 在模型回答前可以附加什麼，以及模型在本輪中可以按需擷取什麼。",
  "Control": "控制項",
  "Modes": "模式",
  ...
  "On usually means OpenWand gathers that source before sending the prompt. Let model decide exposes a tool instead, so the model can fetch the source only if the answer needs it. More context can improve answers, but it may add local parsing work, token usage, network calls, or privacy warnings depending on the source.": "<strong>開</strong>通常表示 OpenWand 在傳送提示詞前收集該來源。<strong>由模型決定</strong>則改為公開一個工具，使模型僅在回答需要時才擷取該來源。更多情境可以改善回答，但依來源不同，可能增加本機解析工作、token 用量、網路呼叫或隱私警告。",
...
Object.assign(I18N.reg['zh-Hant'].tr, {
  ...
  "Open-source coding and agent workflows, especially when GLM is exposed through an API route OpenWand can call.": "開源程式設計與代理工作流，尤其是當 GLM 透過 OpenWand 能呼叫的 API 路由公開時。",
  ...
  "One token for many free backends; point OpenWand's custom endpoint at your local deployment.": "一個 token 對應多個免費後端；將 OpenWand 的自訂端點指向你的本機部署。",
  ...
  "One local endpoint for many backends; point OpenWand's custom endpoint at OmniRoute and use a model such as auto.": "一個本機端點連接多個後端；將 OpenWand 的自訂端點指向 OmniRoute，並使用例如 <code>auto</code> 的模型。",
  "Trial credits are useful for evaluating a model before paying, but they are usually spend-limited or time-limited. Use them for comparison runs; build daily OpenWand usage on a permanent free tier, a paid key, or a local model.": "試用額度適合在付費前評估模型，但通常有消費或時間限制。用它們做對比測試；日常的 OpenWand 使用應建立在長期免費額度、付費金鑰或本機模型之上。",
  ...
  "OpenWand reaches most of these through its OpenAI-compatible client. Many now have a dedicated LLM_PROVIDER value; account-specific or deployment-specific routes still work through the custom endpoint if the provider exposes an OpenAI-compatible URL. Providers without that shape are usually easiest through OpenRouter or another compatible gateway. Add the key itself in Settings → LLM, where it is stored in the OS keychain.": "OpenWand 透過其相容 OpenAI 的用戶端存取其中大多數。許多供應商現在都有專用的 <code>LLM_PROVIDER</code> 值；如果供應商公開了相容 OpenAI 的 URL，帳戶專屬或部署專屬的路由仍可透過 <code>custom</code> 端點運作。沒有這種形式的供應商通常透過 OpenRouter 或其他相容閘道最為簡便。金鑰本身使在 <strong>設定 → LLM</strong> 中填寫，它會儲存到作業系統金鑰鏈中。",
  ...
  "Agent-specific offers such as FreeBuff's free GLM access are not automatically OpenWand API providers. OpenWand needs an API key, a compatible gateway, or a local OpenAI-compatible server.": "面向代理的專屬優惠（例如 FreeBuff 的免費 GLM 存取）並不會自動成為 OpenWand 的 API 供應商。OpenWand 需要 API 金鑰、相容閘道或本機相容 OpenAI 的伺服器。",
...
Object.assign(I18N.reg['zh-Hant'].tr, {
  ...
  "The intent overlay and chat window use the same context-policy shape, so the chips you see before sending match the sources OpenWand is allowed to send or expose.":
    "意圖覆蓋介面和聊天視窗使用相同的情境政策格式，因此你在送出前看到的情境晶片，會對應到 OpenWand 被允許傳送或公開的來源。",
  "When browser context is enabled, OpenWand can fetch the active browser tab, parse HTML with beautifulsoup4, and strip nav/header/footer boilerplate. Private/local URLs are skipped. If the caller uses model-decidable browser context, the page is fetched only when the model calls the context tool.":
    "啟用瀏覽器情境時，OpenWand 可以擷取作用中的瀏覽器分頁、用 <code>beautifulsoup4</code> 解析 HTML，並移除導覽列、頁首與頁尾等樣板內容。私人或本機 URL 會被略過。如果呼叫器使用可由模型決定的瀏覽器情境，頁面只會在模型呼叫情境工具時才被擷取。",

  "Read selected text aloud": "朗讀選取文字",
  "Press F7 to read the current selection aloud with the configured TTS provider. This is separate from model replies: it does not send a query, does not save chat, and is useful when you want OpenWand to read text from the app you are already using.":
    "按下 <kbd>F7</kbd> 可使用已設定的 TTS 供應商朗讀目前選取內容。這和模型回覆是分開的：它不會送出查詢、不會儲存聊天；當你希望 OpenWand 朗讀目前應用程式中的文字時很有用。",
  "Auto-spoken replies are opt-in. OpenWand can stream answers into the bubble silently by default, or speak them as they arrive when you enable reply auto-speak in Settings.":
    "自動朗讀回覆是選用功能。OpenWand 預設可以安靜地將答案串流到泡泡中；若你在設定中啟用回覆自動朗讀，它也可以在答案抵達時同步唸出來。"
});
