...
Object.assign(I18N.reg['fr'].tr, {
  ...
  'tsing a free source in OpenWand': 'ttiliser une source gratuite dans OpenWand',
  ...
  "OpenWand is free, but it still needs a model provider to answer your queries. You don't have to begin with a paid API key — several providers offer a genuinely free tier, free monthly credits, or no-cost rate-limited access. This page rounds up the current options and shows how to connect each one to OpenWand.":
    "OpenWand est gratuit, mais il a tout de même besoin d'un fournisseur de modèle pour répondre à vos requêtes. Vous n'êtes pas obligé de commencer avec une clé d'API payante — plusieurs fournisseurs proposent une offre réellement gratuite, des crédits mensuels offerts, ou un accès sans frais limité en débit. Cette page recense les options actuelles et montre comment connecter chacune à OpenWand.",
  ...
  "OpenWand reaches most of these through its OpenAI-compatible client. A few have a dedicated LLM_PROVIDER value; everything else works through the custom endpoint by pointing CtSTOM_BASE_tRL at the provider's OpenAI-compatible tRL. Add the key itself in Settings → LLM, where it is stored in the OS keychain.":
    "OpenWand atteint la plupart d'entre eux via son client compatible OpenAI. Quelques-uns ont une valeur <code>LLM_PROVIDER</code> dédiée ; tout le reste fonctionne via le point de terminaison <code>custom</code> en faisant pointer <code>CtSTOM_BASE_tRL</code> vers l'tRL compatible OpenAI du fournisseur. Saisissez la clé elle-même dans <strong>Réglages → LLM</strong>, où elle est stockée dans le trousseau du système.",
  'If you run a model on your own machine there are no tokens to bill and nothing leaves the device. Ollama, LM Studio, and vLLM all expose an OpenAI-compatible server that OpenWand talks to through the custom provider.':
    "Si vous exécutez un modèle sur votre propre machine, il n'y a aucun token à facturer et rien ne quitte l'appareil. <strong>Ollama</strong>, <strong>LM Studio</strong> et <strong>vLLM</strong> exposent tous un serveur compatible OpenAI auquel OpenWand s'adresse via le fournisseur <code>custom</code>.",
  ...
  'Front-end browser SDK only — it is not a backend API OpenWand can call.':
    "SDK navigateur côté front uniquement — ce n'est pas une API backend que OpenWand peut appeler.",
  ...
  "Some free tiers may use your prompts to improve their models — don't send sensitive context to them. OpenWand's redaction still applies either way.":
    "Certaines offres gratuites peuvent utiliser vos invites pour améliorer leurs modèles — ne leur envoyez pas de contexte sensible. Le <a onclick=\"navigate('security')\">caviardage</a> de OpenWand s'applique de toute façon.",
  ...
  "Puter.js is a browser SDK, not a server API, so it can't be set as a OpenWand LLM_PROVIDER.":
    "Puter.js est un SDK navigateur, pas une API serveur, il ne peut donc pas être défini comme <code>LLM_PROVIDER</code> OpenWand.",
  ...
  'OpenWand lives as a small animated icon in the corner of your screen — always on top, never in your way. Press the hotkey and a quick picker drops in; choose an action or type your own, and OpenWand grabs the right context, streams the reply, and can read it aloud word by word.':
    "OpenWand vit sous la forme d'une petite icône animée dans un coin de votre écran — toujours au premier plan, jamais gênante. Pressez le raccourci et un sélecteur rapide apparaît ; choisissez une action ou tapez la vôtre, et OpenWand saisit le bon contexte, diffuse la réponse et peut la lire à voix haute, mot à mot.",
  'Any app': 'Toute appli',
  ...
  'OpenWand listens for your custom hotkey across apps, opens with minimal prompt delay, and sends the selected context without a mouse or window switch.':
    "OpenWand écoute votre raccourci personnalisé dans toutes les applis, ouvre l'invite avec un délai minimal et envoie le contexte sélectionné sans souris ni changement de fenêtre.",
  ...
  'When CALLER_N_PASTE_BACK=True, OpenWand pastes the reply straight into whichever input had focus before the overlay opened — replacing the selected text.':
    "Quand <code>CALLER_N_PASTE_BACK=True</code>, OpenWand colle la réponse directement dans le champ qui avait le focus avant l'ouverture de la surimpression — en remplaçant le texte sélectionné.",
  ...
  'OpenWand uses pynput (no admin rights) for caller hotkeys. If a hotkey is already claimed by Windows or another app, OpenWand will not intercept it reliably. Choose combinations that are not globally reserved.':
    "OpenWand utilise <code>pynput</code> (sans droits administrateur) pour les raccourcis d'appelant. Si un raccourci est déjà revendiqué par Windows ou une autre application, OpenWand ne l'interceptera pas de façon fiable. Choisissez des combinaisons qui ne sont pas réservées globalement.",
  ...
  'The reply bubble is a transparent, always-on-top Qt window owned by the openwand-ui worker. Visual properties can be edited in Settings; source checkouts can also edit the same values in .env:':
    "La bulle de réponse est une fenêtre Qt transparente et toujours au premier plan, détenue par le worker <code>openwand-ui</code>. Les propriétés visuelles peuvent être modifiées dans les Réglages ; les versions lancées depuis le code source peuvent aussi modifier les mêmes valeurs dans <code>.env</code> :",
  ...
  'Groq exposes an OpenAI-compatible API so OpenWand uses the openai Python package to talk to it. It is a good choice for latency-sensitive hotkey queries thanks to its low time-to-first-token.':
    "Groq expose une API compatible OpenAI, OpenWand utilise donc le paquet Python <code>openai</code> pour communiquer avec lui. C'est un bon choix pour les requêtes par raccourci sensibles à la latence grâce à son faible temps jusqu'au premier token.",
  ...
  'Model availability depends on your subscription and what the OAuth route exposes to OpenWand. Start with the default shown in Settings, then adjust only if the selected model is available on your account.':
    "La disponibilité des modèles dépend de votre abonnement et de ce que la route OAuth expose à OpenWand. Commencez avec la valeur par défaut affichée dans les Réglages, puis ajustez seulement si le modèle choisi est disponible sur votre compte.",
  ...
  'OpenWand uses the openai Python package for all OpenAI-compatible endpoints. The following providers work by setting the right LLM_PROVIDER value and adding the API key in Settings:':
    "OpenWand utilise le paquet Python <code>openai</code> pour tous les points de terminaison compatibles OpenAI. Les fournisseurs suivants fonctionnent en définissant la bonne valeur <code>LLM_PROVIDER</code> et en ajoutant la clé d'API dans les Réglages :",
  ...
  'Some antivirus products flag keyboard hooks. You may need to add the app directory or OpenWand.exe to your AV exclusion list.':
    "Certains antivirus signalent les hooks <code>keyboard</code>. Vous devrez peut-être ajouter le dossier de l'application ou <code>OpenWand.exe</code> à votre liste d'exclusions antivirus.",
  ...
  'OpenWand runs natively on macOS 13 (Ventura) and later, on both Apple Silicon and Intel Macs. The overlay, voice, context capture, and memory are all supported.':
    "OpenWand s'exécute nativement sur macOS 13 (Ventura) et ultérieur, sur Mac Apple Silicon comme Intel. La surimpression, la voix, la capture de contexte et la mémoire sont toutes prises en charge.",
  ...
  'macOS gates input and screen APIs behind the privacy system (TCC). On first run, grant OpenWand the following under System Settings → Privacy & Security:':
    "macOS protège les API d'entrée et d'écran derrière le système de confidentialité (TCC). Au premier lancement, accordez à OpenWand les éléments suivants dans <strong>Réglages Système → Confidentialité et sécurité</strong> :",
  ...
  'Input Monitoring — required for the global hotkey listener (a purpose-built PyObjC/Carbon backend in openwand-native)':
    "<strong>Surveillance de la saisie</strong> — requise pour l'écouteur de raccourcis globaux (un backend PyObjC/Carbon dédié dans <code>openwand-native</code>)",
  ...
  'macOS only applies new Accessibility / Input Monitoring grants to a process after it is relaunched. Quit and reopen OpenWand once permissions are checked.':
    "macOS n'applique les nouvelles autorisations d'Accessibilité / Surveillance de la saisie à un processus qu'après son relancement. Quittez et rouvrez OpenWand une fois les autorisations cochées.",
  ...
  'If something misbehaves, attach the latest files from build_logs/ to a bug report.':
    "En cas de problème, joignez les fichiers les plus récents de <code>build_logs/</code> à un rapport de bug.",
  'For a session that keeps full runtime logs, start OpenWand with Start OpenWand Debug.command instead of the normal launcher.':
    "Pour une session qui conserve l'intégralité des journaux d'exécution, démarrez OpenWand avec <code>Start OpenWand Debug.command</code> au lieu du lanceur normal.",
  ...
  'OpenWand is best supported on X11 sessions today. We are currently working on Linux Wayland support; native hotkey, clipboard, and screen capture behavior still depends on the desktop environment.':
    "OpenWand est le mieux pris en charge aujourd'hui sur les sessions X11. Nous travaillons actuellement sur la prise en charge de Linux Wayland ; le comportement natif des raccourcis, du presse-papiers et de la capture d'écran dépend encore de l'environnement de bureau.",
  ...
  'OpenWand records approval for the exact dependency set, so an update that changes packages must be approved again before it runs. uv is used when available, falling back to python -m venv in source checkouts.':
    "OpenWand enregistre l'approbation pour l'ensemble exact de dépendances, de sorte qu'une mise à jour modifiant les paquets doit être approuvée à nouveau avant de s'exécuter. <code>uv</code> est utilisé lorsqu'il est disponible, avec repli sur <code>python -m venv</code> dans les checkouts source.",
  ...
  'Distribution is supported with .zip or .openwand archives containing one add-on folder; the Addon Manager can also install from an unpacked folder.':
    "La distribution est prise en charge via des archives <code>.zip</code> ou <code>.openwand</code> contenant un dossier de module ; le Gestionnaire de modules peut aussi installer depuis un dossier décompressé.",
  ...
  'Script tools in tools/installed/ still load, but the supported way to extend OpenWand is now Add-ons — they run in isolated processes and do far more than register a tool.':
    "Les scripts-outils dans <code>tools/installed/</code> se chargent toujours, mais la manière prise en charge d'étendre OpenWand est désormais les <a onclick=\"navigate('addons')\">modules complémentaires</a> — ils s'exécutent dans des processus isolés et font bien plus qu'enregistrer un outil.",
  ...
  'Tool plugins run in the same process as OpenWand with full OS access. Only install plugins you trust.':
    "Les plugins d'outils s'exécutent dans le même processus que OpenWand avec un accès complet au système. N'installez que des plugins de confiance.",
  ...
  "Define at least one LLM_FALLBACKS route so a single provider outage or rate limit doesn't break your hotkeys — OpenWand tries each route in order.":
    "Définissez au moins une route <code>LLM_FALLBACKS</code> pour qu'une panne d'un seul fournisseur ou une limite de débit ne casse pas vos raccourcis — OpenWand essaie chaque route dans l'ordre.",
  ...
  'Keep the contents of dist/OpenWand/ together when moving the portable build to another folder or machine.':
    "Gardez le contenu de <code>dist/OpenWand/</code> ensemble lorsque vous déplacez le build portable vers un autre dossier ou une autre machine.",
  ...
  'On your machine. Settings, chats, memory, privacy reports, and local configuration are written to local app data paths, not to a OpenWand-hosted account.':
    "Sur votre machine. Les réglages, chats, mémoires, rapports de confidentialité et la configuration locale sont écrits dans les chemins de données locaux de l'application, pas dans un compte hébergé par OpenWand.",
  ...
  'It is the secure password store built into your operating system: Windows Credential Manager on Windows, Keychain on macOS, and Secret Service or KWallet on many Linux desktops. OpenWand uses it for provider keys and OAuth tokens instead of writing them into .env or a plain config file.':
    "C'est le stockage sécurisé de mots de passe intégré à votre système d'exploitation : Gestionnaire d'identifiants sous Windows, Trousseau sous macOS, et Secret Service ou KWallet sur de nombreux bureaux Linux. OpenWand l'utilise pour les clés fournisseur et les tokens OAuth au lieu de les écrire dans <code>.env</code> ou dans un fichier de configuration en clair.",
  'Does OpenWand send everything on my screen?': "OpenWand envoie-t-il tout ce qui est sur mon écran ?",
  ...
  'Use the portable package for your OS: Windows .exe, macOS app or launcher, or Linux portable build or launcher. If you are running from the repo, use Start OpenWand.bat, Start OpenWand.command, or Start OpenWand.sh; the first source run installs dependencies, and later runs just launch the app.':
    "Utilisez le paquet portable pour votre système : le <code>.exe</code> Windows, l'app ou le lanceur macOS, ou le build portable ou lanceur Linux. Si vous lancez depuis le dépôt, utilisez <code>Start OpenWand.bat</code>, <code>Start OpenWand.command</code> ou <code>Start OpenWand.sh</code> ; le premier lancement depuis les sources installe les dépendances, puis les suivants démarrent simplement l'application.",
  ...
  'Yes, if they expose an OpenAI-compatible endpoint. Ollama works through its /v1 endpoint, and LM Studio / vLLM can be used through the custom endpoint route. OpenWand does not directly speak native, non-OpenAI-compatible local model APIs.':
    "Oui, s'ils exposent un endpoint compatible OpenAI. Ollama fonctionne via son endpoint <code>/v1</code>, et LM Studio / vLLM peuvent être utilisés via la route d'endpoint personnalisé. OpenWand ne parle pas directement aux APIs locales natives non compatibles OpenAI.",
  ...
  'Yes. Set a primary route and optional fallback routes so OpenWand can switch when a provider is unavailable or limited.':
    "Oui. Définissez une route principale et des routes de secours facultatives afin que OpenWand puisse basculer quand un fournisseur est indisponible ou limité.",
  ...
  'Provider capabilities differ. OpenWand shows model warnings when the selected route does not support a feature needed by the current request.':
    "Les capacités varient selon le fournisseur. OpenWand affiche des avertissements quand la route sélectionnée ne prend pas en charge une fonction nécessaire à la requête actuelle.",
  ...
  'Is OpenWand free?': 'OpenWand est-il gratuit ?',
  'Yes. OpenWand is free and open source. You may still pay for any model provider, TTS provider, or hosted service you choose to connect.':
    "Oui. OpenWand est gratuit et open source. Vous pouvez toutefois payer le fournisseur de modèle, le fournisseur TTS ou le service hébergé que vous choisissez de connecter.",
  ...
  'OpenWand falls back to CPU automatically when CUDA is not compatible.':
    'OpenWand repasse automatiquement sur CPU lorsque CUDA n’est pas compatible.',
  'Details': 'Détails',
  ...
  'The log says Kokoro model failed on cuda: cuDNN version 91900 is not compatible with devices with SM < 7.5, then OpenWand falls back to CPU.':
    'Le journal indique que le modèle Kokoro a échoué sur <code>cuda</code> : la version cuDNN 91900 n’est pas compatible avec les appareils dont le SM est inférieur à 7.5, puis OpenWand repasse sur CPU.',
  ...
  'OpenWand already falls back safely. We may add a preflight CUDA capability check so unsupported GPUs skip the failed CUDA attempt.':
    'OpenWand effectue déjà un repli sûr. Nous pourrons ajouter une vérification préalable de la capacité CUDA afin que les GPU non pris en charge évitent la tentative CUDA vouée à l’échec.',
  ...
  'Install Python 3.12, then rerun Start OpenWand.command. macOS installs from requirements/requirements-macos.lock.':
    "Installez Python <code>3.12</code>, puis relancez <code>Start OpenWand.command</code>. macOS installe depuis <code>requirements/requirements-macos.lock</code>.",
  ...
  'Enable Browser/Web context for the caller. If the chip says deferred, OpenWand may fetch page text only after you send.':
    "Activez le contexte Navigateur/Web pour l'appelant. Si la pastille indique différé, OpenWand peut récupérer le texte de la page seulement après l'envoi.",
  ...
  'Grant Accessibility, Screen Recording, and Microphone permissions as needed, then restart OpenWand.':
    "Accordez les autorisations Accessibilité, Enregistrement de l'écran et Microphone selon les besoins, puis redémarrez OpenWand.",
  'Linux': 'Linux',
...
Object.assign(I18N.reg['fr'].tr, {
  ...
  'The core OpenWand loop: press the hotkey, choose an intent, send selected or enabled context, and read the streamed answer without leaving the active app.':
    "La boucle de base de OpenWand : appuyer sur le raccourci, choisir une intention, envoyer le contexte sélectionné ou activé, puis lire la réponse en streaming sans quitter l'application active.",
  ...
  'When visual context matters, draw a region with Ctrl Alt Q. OpenWand sends only that crop to a vision-capable model and keeps the response in the overlay.':
    "Quand le contexte visuel compte, tracez une zone avec <kbd>Ctrl Alt Q</kbd>. OpenWand n'envoie que cette découpe à un modèle compatible vision et garde la réponse dans la surimpression.",
  ...
  'OpenWand hotkey overlay query demo': 'Démo de requête OpenWand avec raccourci dans la surimpression',
  'OpenWand screen snip demo': "Démo de capture d'écran OpenWand",
  'OpenWand context-aware rewrite demo': 'Démo de réécriture OpenWand avec contexte',
  'OpenWand multi-agent task demo': 'Démo de tâche multi-agent OpenWand',
...
Object.assign(I18N.reg['fr'].tr, {
  ...
  "On usually means OpenWand gathers that source before sending the prompt. Let model decide exposes a tool instead, so the model can fetch the source only if the answer needs it. More context can improve answers, but it may add local parsing work, token usage, network calls, or privacy warnings depending on the source.": "<strong>Activé</strong> signifie généralement que OpenWand collecte cette source avant d'envoyer l'invite. <strong>Laisser le modèle décider</strong> expose plutôt un outil, afin que le modèle ne récupère la source que si la réponse en a besoin. Plus de contexte peut améliorer les réponses, mais cela peut ajouter du travail d'analyse local, de la consommation de tokens, des appels réseau ou des avertissements de confidentialité selon la source.",
...
Object.assign(I18N.reg['fr'].tr, {
  ...
  "Open-source coding and agent workflows, especially when GLM is exposed through an API route OpenWand can call.": "Workflows de code et d'agents open source, surtout lorsque GLM est exposé via une route d'API que OpenWand peut appeler.",
  ...
  "One token for many free backends; point OpenWand's custom endpoint at your local deployment.": "tn seul token pour de nombreux backends gratuits ; faites pointer le point de terminaison personnalisé de OpenWand vers votre déploiement local.",
  ...
  "One local endpoint for many backends; point OpenWand's custom endpoint at OmniRoute and use a model such as auto.": "tn seul point de terminaison local pour de nombreux backends ; faites pointer le point de terminaison personnalisé de OpenWand vers OmniRoute et utilisez un modèle comme <code>auto</code>.",
  "Local — Ollama / LM Studio / vLLM": "Local — Ollama / LM Studio / vLLM",
  "Trial credits are useful for evaluating a model before paying, but they are usually spend-limited or time-limited. Use them for comparison runs; build daily OpenWand usage on a permanent free tier, a paid key, or a local model.": "Les crédits d'essai sont utiles pour évaluer un modèle avant de payer, mais ils sont généralement limités en dépense ou en durée. Utilisez-les pour des comparaisons ; construisez votre usage quotidien de OpenWand sur une offre gratuite permanente, une clé payante ou un modèle local.",
  ...
  "Agent-specific offers such as FreeBuff's free GLM access are not automatically OpenWand API providers. OpenWand needs an API key, a compatible gateway, or a local OpenAI-compatible server.": "Les offres spécifiques aux agents, comme l'accès gratuit à GLM de FreeBuff, ne sont pas automatiquement des fournisseurs d'API pour OpenWand. OpenWand a besoin d'une clé d'API, d'une passerelle compatible ou d'un serveur local compatible OpenAI.",
...
Object.assign(I18N.reg['fr'].tr, {
  "Sensitive text filtered before cloud sends": "Texte sensible filtré avant tout envoi vers le cloud",
  "Built-in patterns — and optional local AI — replace likely sensitive text before cloud-bound requests are sent.": "Les règles intégrées — et l'IA locale facultative — remplacent le texte probablement sensible avant l'envoi des requêtes vers le cloud.",
  "Choose your privacy mode": "Choisissez votre mode de confidentialité",
  "OpenWand offers three mutually exclusive settings under Settings → App → Privacy mode:": "OpenWand propose trois options mutuellement exclusives dans <strong>Paramètres → Application → Mode de confidentialité</strong> :",
  "Mode": "Mode",
  "What it does": "Fonctionnement",
  "Off": "<strong>Désactivé</strong>",
  "No automatic privacy filtering.": "Aucun filtrage automatique de confidentialité.",
  "Built-in (default)": "<strong>Intégré</strong> (par défaut)",
  "Local pattern detection for credentials, tokens, payment details, and other structured secrets.": "Détection locale par règles des identifiants, jetons, données de paiement et autres secrets structurés.",
  "Advanced": "<strong>Avancé</strong>",
  "The built-in detector plus a local, context-aware AI model for names, addresses, email addresses, phone numbers, private URLs and dates, account numbers, and secrets.": "Le détecteur intégré, complété par un modèle d'IA local sensible au contexte pour les noms, adresses, adresses e-mail, numéros de téléphone, URL et dates privées, numéros de compte et secrets.",
  "Advanced privacy runs locally": "La confidentialité avancée s'exécute localement",
...
Object.assign(I18N.reg['fr'].nav.labels, {
  "ChatGPT, Codex & Claude CLI": "ChatGPT, Codex et Claude CLI",
});
Object.assign(I18N.reg['fr'].meta, {
  "live-agents": {
    title: "ChatGPT, Codex et Claude CLI",
    sub: "Exécutez des sessions complètes d’agents locaux derrière OpenWand, avec progression en direct, approbations et continuité reprenable.",
  },
});
Object.assign(I18N.reg['fr'].tr, {
  "OpenWand": "OpenWand",
  ...
  "Visible reasoning": "Raisonnement visible",
  "Permission mode": "Mode d’autorisation",
  "How the agent modes work": "Fonctionnement des modes agent",
  "OpenWand can use more than a model API. It can run a complete local coding-agent harness behind both the overlay and the full chat window. The ChatGPT option starts the installed Codex CLI in app-server mode; Claude Agent uses the Claude Agent SDK and the same Claude Code CLI authentication. OpenWand supplies the prompt and captured context, then streams the agent's visible output back into the normal OpenWand interface.": "OpenWand peut faire plus qu’utiliser une API de modèle. Il peut exécuter un agent de programmation local complet derrière la superposition comme la fenêtre de chat. L’option <strong>ChatGPT</strong> démarre la CLI Codex installée en mode app-server ; <strong>Claude Agent</strong> utilise le SDK Claude Agent et la même authentification que la CLI Claude Code. OpenWand fournit le prompt et le contexte capturé, puis diffuse la sortie visible de l’agent dans l’interface OpenWand habituelle.",
  "Run conversations with": "Exécuter les conversations avec",
  "Behavior": "Comportement",
  "Uses the LLM provider and model configured in OpenWand.": "Utilise le fournisseur LLM et le modèle configurés dans OpenWand.",
  "Runs the local Codex CLI app-server and uses your ChatGPT/Codex account.": "Exécute l’app-server local de la CLI Codex et utilise votre compte ChatGPT/Codex.",
  "Runs the Claude Agent SDK with the Claude Code CLI and uses your Claude account.": "Exécute le SDK Claude Agent avec la CLI Claude Code et utilise votre compte Claude.",
  "One OpenWand interface": "Une seule interface OpenWand",
  "Hotkey queries and full-chat prompts use the selected engine. OpenWand still captures context, applies privacy controls, displays the stream, and stores a local copy of the conversation.": "Les requêtes par raccourci et les prompts du chat complet utilisent le moteur sélectionné. OpenWand continue de capturer le contexte, d’appliquer les contrôles de confidentialité, d’afficher le flux et de conserver une copie locale de la conversation.",
  "Select and sign in": "Sélectionner et se connecter",
  "Open Settings → App → Run conversations with and choose OpenWand, ChatGPT, or Claude Agent. When an agent is selected, the same card shows its login status and provides Sign in, Sign out, and Refresh actions. Sign-in opens the provider's terminal and browser flow.": "Ouvrez <strong>Paramètres → Application → Exécuter les conversations avec</strong>, puis choisissez OpenWand, ChatGPT ou Claude Agent. Lorsqu’un agent est sélectionné, la même carte affiche l’état de connexion et propose les actions <strong>Se connecter</strong>, <strong>Se déconnecter</strong> et <strong>Actualiser</strong>. La connexion ouvre le parcours terminal et navigateur du fournisseur.",
  "ChatGPT mode requires the Codex CLI. OpenWand gives Codex an isolated local profile, so its login and resumable OpenWand sessions do not appear in your personal Codex history. Claude Agent uses the bundled Claude Agent SDK when available and authenticates through the Claude Code CLI.": "Le mode ChatGPT nécessite la CLI Codex. OpenWand attribue à Codex un profil local isolé : sa connexion et les sessions OpenWand reprenables n’apparaissent donc pas dans votre historique Codex personnel. Claude Agent utilise le SDK Claude Agent inclus lorsqu’il est disponible et s’authentifie via la CLI Claude Code.",
  "Choose who owns continuity": "Choisir qui gère la continuité",
  "The Conversation goes to setting decides how follow-up prompts carry context:": "Le réglage <strong>Conversation envoyée à</strong> détermine comment les prompts suivants conservent le contexte :",
  "Owner": "Responsable",
  "What happens": "Ce qui se passe",
  "Best for": "Idéal pour",
  "OpenWand sends the complete local OpenWand history on every request. Each request starts without a retained provider continuation link.": "OpenWand envoie tout l’historique OpenWand local à chaque requête. Chaque requête démarre sans conserver de lien de continuation avec le fournisseur.",
  "Portable conversations that can change engines without depending on a provider session.": "Les conversations portables pouvant changer de moteur sans dépendre d’une session fournisseur.",
  "ChatGPT or Claude Agent": "ChatGPT ou Claude Agent",
  "OpenWand transfers the existing history once, stores the returned provider session ID, and resumes that session for later prompts.": "OpenWand transfère une fois l’historique existant, enregistre l’identifiant de session renvoyé par le fournisseur et reprend cette session pour les prompts suivants.",
  "Long agent conversations that should preserve native provider state efficiently.": "Les longues conversations d’agent qui doivent conserver efficacement l’état natif du fournisseur.",
  "OpenWand keeps separate OpenWand, ChatGPT/Codex, and Claude conversation namespaces so switching engines does not append a prompt to the wrong provider's history. Changing the project folder also starts a clean provider session.": "OpenWand sépare les espaces de conversation OpenWand, ChatGPT/Codex et Claude afin qu’un changement de moteur n’ajoute pas un prompt à l’historique du mauvais fournisseur. Changer de dossier de projet démarre également une nouvelle session fournisseur.",
  "What appears while the agent works": "Ce qui apparaît pendant le travail de l’agent",
  "Replies stream normally, while visible reasoning summaries, plans, tool starts, command or file progress, status updates, and approval requests appear as progress in the bubble and chat. OpenWand can only show events the provider exposes; private hidden chain-of-thought is never available.": "Les réponses sont diffusées normalement, tandis que les résumés de raisonnement visibles, plans, lancements d’outils, progressions de commandes ou de fichiers, états et demandes d’approbation apparaissent comme progression dans la bulle et le chat. OpenWand ne peut afficher que les événements exposés par le fournisseur ; la chaîne de pensée privée et masquée n’est jamais disponible.",
  "When a command, file change, or other protected tool needs permission, OpenWand presents the action, path, command, or diff it received and sends your decision back to the running agent.": "Lorsqu’une commande, une modification de fichier ou un autre outil protégé nécessite une autorisation, OpenWand présente l’action, le chemin, la commande ou le diff reçu, puis renvoie votre décision à l’agent en cours d’exécution.",
  "Live provider controls": "Contrôles du fournisseur en direct",
  "When ChatGPT or Claude Agent is active, a provider badge appears below the floating OpenWand icon. Click it to change settings for the next turn:": "Lorsque ChatGPT ou Claude Agent est actif, un badge du fournisseur apparaît sous l’icône OpenWand flottante. Cliquez dessus pour modifier les réglages du prochain tour :",
  "Control": "Contrôle",
  "Effect": "Effet",
  "Use the provider default or enter a supported model name.": "Utilisez le modèle par défaut du fournisseur ou saisissez le nom d’un modèle pris en charge.",
  "Choose the folder the agent may inspect or modify, or leave it on Auto.": "Choisissez le dossier que l’agent peut inspecter ou modifier, ou laissez le réglage sur Auto.",
  "Requests the provider's faster tier when supported; it may use more quota or cost more.": "Demande le niveau rapide du fournisseur lorsqu’il est pris en charge ; il peut consommer davantage de quota ou coûter plus cher.",
  "Sets the effort level passed to the provider when supported.": "Définit le niveau d’effort transmis au fournisseur lorsqu’il est pris en charge.",
  "Choose detailed or concise Codex summaries, provider Claude summaries, or turn summaries off.": "Choisissez des résumés Codex détaillés ou concis, les résumés Claude du fournisseur, ou désactivez les résumés du tour.",
  "Ask for permission, allow changes inside the project, or keep the agent in plan-only read-only mode.": "Demandez une autorisation, permettez les modifications dans le projet ou maintenez l’agent en mode planification uniquement et lecture seule.",
  "Older provider versions that do not support a reasoning or approval option are retried with compatible settings when possible.": "Les anciennes versions du fournisseur qui ne prennent pas en charge une option de raisonnement ou d’approbation sont relancées avec des réglages compatibles lorsque cela est possible.",
  "Project and context behavior": "Comportement du projet et du contexte",
  "In Auto mode, OpenWand reuses the resumed session's folder, then looks at attached files and file context, and finally falls back to OpenWand's current directory. A manually selected project overrides that inference. Agent write access remains limited to the selected project, and Codex runs without network access in its workspace sandbox.": "En mode Auto, OpenWand réutilise le dossier de la session reprise, examine ensuite les pièces jointes et le contexte de fichiers, puis utilise en dernier recours le répertoire courant de OpenWand. Un projet sélectionné manuellement remplace cette déduction. L’accès en écriture de l’agent reste limité au projet sélectionné et Codex s’exécute sans accès réseau dans le bac à sable de son espace de travail.",
  "Selected text, ambient app context, attachments, memory, and privacy-filtered content are assembled by OpenWand before the turn. Provider-specific system prompts stay in the native harness instructions instead of being pasted into your message.": "OpenWand rassemble avant le tour le texte sélectionné, le contexte ambiant de l’application, les pièces jointes, la mémoire et le contenu filtré pour la confidentialité. Les prompts système propres au fournisseur restent dans les instructions natives de l’agent au lieu d’être collés dans votre message.",
  "Offline transcript fallback": "Solution de repli par transcriptions hors ligne",
  "Live agent sessions are the normal path. OpenWand also provides local transcript pull, push, and export as an experimental compatibility fallback. Pull discovers Codex and Claude JSONL histories on disk without contacting either provider. Push requires confirmation, creates a full backup, and appends only OpenWand-only turns to an existing transcript. Export requires confirmation and creates a new provider transcript without overwriting existing history.": "Les sessions d’agent en direct constituent le parcours normal. OpenWand propose aussi l’importation, le renvoi et l’exportation de transcriptions locales comme solution expérimentale de compatibilité. L’importation découvre sur le disque les historiques JSONL de Codex et Claude sans contacter les fournisseurs. Le renvoi exige une confirmation, crée une sauvegarde complète et ajoute uniquement les tours propres à OpenWand à une transcription existante. L’exportation exige une confirmation et crée une nouvelle transcription fournisseur sans écraser l’historique existant.",
  "Local files, separate from live sessions": "Fichiers locaux, séparés des sessions en direct",
  "Transcript sync edits local compatibility files; it does not send a live request or replace the supported Codex app-server and Claude Agent SDK session flow.": "La synchronisation des transcriptions modifie des fichiers locaux de compatibilité ; elle n’envoie pas de requête en direct et ne remplace pas le parcours de session pris en charge par l’app-server Codex et le SDK Claude Agent.",
});
