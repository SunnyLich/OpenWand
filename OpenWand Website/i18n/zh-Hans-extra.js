...
Object.assign(I18N.reg['zh-Hans'].tr, {
  ...
  'Using a free source in OpenWand': '在 OpenWand 中使用免费来源',
  ...
  "OpenWand is free, but it still needs a model provider to answer your queries. You don't have to begin with a paid API key — several providers offer a genuinely free tier, free monthly credits, or no-cost rate-limited access. This page rounds up the current options and shows how to connect each one to OpenWand.":
    "OpenWand 是免费的，但它仍然需要一个模型提供方来回答你的查询。你不必一开始就使用付费 API 密钥——多个提供方提供真正的免费额度、每月赠送额度或限速的零成本访问。本页汇总了当前的选项，并说明如何将每一个连接到 OpenWand。",
  ...
  "OpenWand reaches most of these through its OpenAI-compatible client. A few have a dedicated LLM_PROVIDER value; everything else works through the custom endpoint by pointing CUSTOM_BASE_URL at the provider's OpenAI-compatible URL. Add the key itself in Settings → LLM, where it is stored in the OS keychain.":
    "OpenWand 通过其兼容 OpenAI 的客户端访问其中大多数。少数提供方有专用的 <code>LLM_PROVIDER</code> 值；其余的都通过 <code>custom</code> 端点工作，只需将 <code>CUSTOM_BASE_URL</code> 指向提供方兼容 OpenAI 的 URL。密钥本身使在 <strong>设置 → LLM</strong> 中填写，它会保存到操作系统密钥链中。",
  'If you run a model on your own machine there are no tokens to bill and nothing leaves the device. Ollama, LM Studio, and vLLM all expose an OpenAI-compatible server that OpenWand talks to through the custom provider.':
    "如果你在自己的机器上运行模型，就没有 token 需要计费，也没有任何数据离开设备。<strong>Ollama</strong>、<strong>LM Studio</strong> 和 <strong>vLLM</strong> 都会暴露一个兼容 OpenAI 的服务器，OpenWand 通过 <code>custom</code> 提供方与之通信。",
  ...
  'Front-end browser SDK only — it is not a backend API OpenWand can call.':
    "仅为前端浏览器 SDK——它不是 OpenWand 可以调用的后端 API。",
  ...
  "Some free tiers may use your prompts to improve their models — don't send sensitive context to them. OpenWand's redaction still applies either way.":
    "有些免费额度可能会用你的提示词来改进其模型——不要向它们发送敏感上下文。无论如何，OpenWand 的<a onclick=\"navigate('security')\">脱敏</a>依然生效。",
  ...
  "Puter.js is a browser SDK, not a server API, so it can't be set as a OpenWand LLM_PROVIDER.":
    "Puter.js 是浏览器 SDK，而非服务器 API，因此不能设为 OpenWand 的 <code>LLM_PROVIDER</code>。",
  ...
  'OpenWand lives as a small animated icon in the corner of your screen — always on top, never in your way. Press the hotkey and a quick picker drops in; choose an action or type your own, and OpenWand grabs the right context, streams the reply, and can read it aloud word by word.':
    'OpenWand 以一个小巧的动态图标停在屏幕角落——始终置顶，却从不碍事。按下热键，快捷选择器随即出现；选择一个操作或自行输入，OpenWand 便会抓取合适的上下文，在约一秒半内作答，并逐字朗读回复。',
  'Any app': '任何应用',
  ...
  'OpenWand listens for your custom hotkey across apps, opens with minimal prompt delay, and sends the selected context without a mouse or window switch.':
    'OpenWand 会在所有应用中监听你的自定义热键，以极低延迟打开提问界面，并发送选中的上下文；无需鼠标，也无需切换窗口。',
  ...
  'When CALLER_N_PASTE_BACK=True, OpenWand pastes the reply straight into whichever input had focus before the overlay opened — replacing the selected text.':
    '当 <code>CALLER_N_PASTE_BACK=True</code> 时，OpenWand 会将回复直接粘贴到悬浮层打开前拥有焦点的输入框中——替换选中的文本。',
  ...
  'OpenWand uses pynput (no admin rights) for caller hotkeys. If a hotkey is already claimed by Windows or another app, OpenWand will not intercept it reliably. Choose combinations that are not globally reserved.':
    'OpenWand 使用 <code>pynput</code>（无需管理员权限）处理调用者热键。如果某个热键已被 Windows 或其他应用占用，OpenWand 将无法可靠地拦截它。使选择未被全局保留的组合。',
  ...
  'The reply bubble is a transparent, always-on-top Qt window owned by the openwand-ui worker. Visual properties can be edited in Settings; source checkouts can also edit the same values in .env:':
    '回复气泡是一个透明、始终置顶的 Qt 窗口，由 <code>openwand-ui</code> worker 拥有。视觉属性可以在设置中编辑；源码版本也可以在 <code>.env</code> 中编辑相同的值：',
  ...
  'Groq exposes an OpenAI-compatible API so OpenWand uses the openai Python package to talk to it. It is a good choice for latency-sensitive hotkey queries thanks to its low time-to-first-token.':
    'Groq 提供与 OpenAI 兼容的 API，因此 OpenWand 使用 <code>openai</code> Python 包与之通信。凭借极低的首字延迟，它是对延迟敏感的热键查询的理想选择。',
  ...
  'This route is stable today, but it depends on OpenAI continuing to allow subscription-backed OAuth access from third-party clients. Provider policy can change later, so keep an API-key, local, or other provider route as a fallback if OpenWand is part of your daily workflow.':
    '这条路线目前稳定，但它依赖 OpenAI 继续允许第三方客户端使用订阅支持的 OAuth 访问。提供方政策以后可能变化，所以如果 OpenWand 是你的日常工作流，请保留 API 密钥、本地模型或其他提供方作为备用路线。',
  ...
  'Model availability depends on your subscription and what the OAuth route exposes to OpenWand. Start with the default shown in Settings, then adjust only if the selected model is available on your account.':
    '模型可用性取决于你的订阅以及 OAuth 路线向 OpenWand 暴露的内容。先使用设置中显示的默认值；只有当所选模型在你的账户中可用时再调整。',
  ...
  'OpenWand uses the openai Python package for all OpenAI-compatible endpoints. The following providers work by setting the right LLM_PROVIDER value and adding the API key in Settings:':
    'OpenWand 对所有兼容 OpenAI 的端点都使用 <code>openai</code> Python 包。以下提供商只需设置正确的 <code>LLM_PROVIDER</code> 值并在设置中添加 API 密钥即可使用：',
  ...
  'Some antivirus products flag keyboard hooks. You may need to add the app directory or OpenWand.exe to your AV exclusion list.':
    '某些杀毒软件会标记 <code>keyboard</code> 钩子。你可能需要将应用目录或 <code>OpenWand.exe</code> 添加到杀毒软件的排除列表中。',
  ...
  'OpenWand runs natively on macOS 13 (Ventura) and later, on both Apple Silicon and Intel Macs. The overlay, voice, context capture, and memory are all supported.':
    'OpenWand 在 macOS 13（Ventura）及更高版本上原生运行，支持 Apple Silicon 和 Intel Mac。悬浮层、语音、上下文捕获和记忆均受支持。',
  ...
  'The packaged macOS build was last live-tested quite a while ago, so it may be buggier than the Windows build or the repo launcher path. If it gives you trouble, please try the repo version with Start OpenWand.command; it is the best-supported macOS path right now. Renting Apple hardware for fresh testing costs money, so if you would like to support more macOS verification, you can donate at Buy Me a Coffee. No pressure either way: clear bug reports with logs are also very helpful.':
    'macOS 打包版距离上次真实机器实测已经有一段时间，因此可能比 Windows 版本或仓库启动器路径更容易出问题。如果遇到问题，使尝试使用 <code>Start OpenWand.command</code> 运行仓库版本；这是目前支持最好的 macOS 路径。租用 Apple 硬件进行新的测试需要费用，所以如果你想支持更多 macOS 验证，可以在 <a href="https://buymeacoffee.com/sunnylich" target="_blank">Buy Me a Coffee</a> 捐助。当然完全没有压力：附带日志的清晰 bug 报告也非常有帮助。',
  'Area': '方面',
  'Full support': '完全支持',
  ...
  'macOS gates input and screen APIs behind the privacy system (TCC). On first run, grant OpenWand the following under System Settings → Privacy & Security:':
    'macOS 通过隐私系统（TCC）对输入和屏幕 API 设限。首次运行时，使在<strong>系统设置 → 隐私与安全性</strong>中授予 OpenWand 以下权限：',
  ...
  'Input Monitoring — required for the global hotkey listener (a purpose-built PyObjC/Carbon backend in openwand-native)':
    '<strong>输入监控</strong>——全局热键监听器所必需（<code>openwand-native</code> 中专门构建的 PyObjC/Carbon 后端）',
  ...
  'macOS only applies new Accessibility / Input Monitoring grants to a process after it is relaunched. Quit and reopen OpenWand once permissions are checked.':
    'macOS 只有在进程重新启动后才会对其应用新的辅助功能 / 输入监控授权。勾选权限后，使退出并重新打开 OpenWand。',
  ...
  'For a session that keeps full runtime logs, start OpenWand with Start OpenWand Debug.command instead of the normal launcher.':
    '若要让会话保留完整的运行日志，请使用 <code>Start OpenWand Debug.command</code> 而非普通启动器来启动 OpenWand。',
  ...
  'OpenWand is best supported on X11 sessions today. We are currently working on Linux Wayland support; native hotkey, clipboard, and screen capture behavior still depends on the desktop environment.':
    'OpenWand 目前在 X11 会话上的支持最好。我们正在推进 Linux Wayland 支持；原生快捷键、剪贴板和屏幕捕获行为仍取决于桌面环境。',
  ...
  'Add-ons are the supported way to extend OpenWand. An add-on can observe or modify query context, observe responses, contribute tray actions, expose settings, register model-callable tools, and declare its own intents and hotkeys.':
    '附加组件是扩展 OpenWand 的受支持方式。附加组件可以观察或修改查询上下文、观察响应、贡献托盘操作、公开设置、注册可供模型调用的工具，并声明自己的意图和热键。',
  ...
  'Each enabled add-on runs in its own Python host process — one process per add-on. A crash, import failure, or slow hook is isolated from the brain worker and from every other add-on. OpenWand talks to each host over a small newline-delimited JSON IPC protocol.':
    '每个启用的附加组件都在<strong>自己的 Python 宿主进程</strong>中运行——每个附加组件一个进程。崩溃、导入失败或缓慢的钩子都会与“大脑” worker 以及所有其他附加组件隔离。OpenWand 通过一个以换行分隔的小型 JSON IPC 协议与每个宿主通信。',
  'Layout': '布局',
  ...
  'Capabilities are opt-in — missing permissions are denied. An add-on without tools = true can\'t register tools; one without ui = ["tray"] can\'t add tray actions. LLM actions require llm = true and are capped by OpenWand before any provider credentials are used.':
    '能力是按需启用的——<strong>缺失的权限会被拒绝</strong>。没有 <code>tools = true</code> 的附加组件无法注册工具；没有 <code>ui = [\"tray\"]</code> 的则无法添加托盘操作。LLM 操作需要 <code>llm = true</code>，并在使用任何提供商凭据之前由 OpenWand 设限。',
  ...
  '[dependencies] is optional. Add-ons without it run from OpenWand\'s own Python runtime. Add-ons that declare packages get a dedicated virtual environment under addon_envs/<id>/; the Addon Manager shows the required packages and offers an Install/Repair action.':
    '<code>[dependencies]</code> 是可选的。没有它的附加组件将从 OpenWand 自己的 Python 运行时运行。声明了软件包的附加组件会在 <code>addon_envs/&lt;id&gt;/</code> 下获得专用的虚拟环境；附加组件管理器会显示所需软件包并提供“安装/修复”操作。',
  ...
  'OpenWand records approval for the exact dependency set, so an update that changes packages must be approved again before it runs. uv is used when available, falling back to python -m venv in source checkouts.':
    'OpenWand 会记录对确切依赖集合的批准，因此更改软件包的更新在运行前必须再次获得批准。可用时使用 <code>uv</code>，在源码检出中回退到 <code>python -m venv</code>。',
  ...
  'Distribution is supported with .zip or .openwand archives containing one add-on folder; the Addon Manager can also install from an unpacked folder.':
    '支持以包含单个附加组件文件夹的 <code>.zip</code> 或 <code>.openwand</code> 归档进行分发；附加组件管理器也可从解压后的文件夹安装。',
  ...
  'Script tools in tools/installed/ still load, but the supported way to extend OpenWand is now Add-ons — they run in isolated processes and do far more than register a tool.':
    '<code>tools/installed/</code> 中的脚本工具仍会加载，但现在扩展 OpenWand 的受支持方式是<a onclick="navigate(\'addons\')">附加组件</a>——它们在隔离进程中运行，所做的远不止注册一个工具。',
  ...
  'Tool plugins run in the same process as OpenWand with full OS access. Only install plugins you trust.':
    '工具插件与 OpenWand 在同一进程中运行，拥有完整的系统访问权限。使只安装你信任的插件。',
  ...
  "Define at least one LLM_FALLBACKS route so a single provider outage or rate limit doesn't break your hotkeys — OpenWand tries each route in order.":
    '至少定义一条 <code>LLM_FALLBACKS</code> 路线，这样单个提供商的故障或速率限制就不会让你的热键失灵——OpenWand 会按顺序尝试每条路线。',
  ...
  'Keep the contents of dist/OpenWand/ together when moving the portable build to another folder or machine.':
    '将便携版构建移动到其他文件夹或机器时，使保持 <code>dist/OpenWand/</code> 内的内容在一起。',
  ...
  'On your machine. Settings, chats, memory, privacy reports, and local configuration are written to local app data paths, not to a OpenWand-hosted account.':
    '在你的机器上。设置、聊天、记忆、隐私报告和本地配置都会写入本地应用数据路径，而不是写入由 OpenWand 托管的账号。',
  ...
  'It is the secure password store built into your operating system: Windows Credential Manager on Windows, Keychain on macOS, and Secret Service or KWallet on many Linux desktops. OpenWand uses it for provider keys and OAuth tokens instead of writing them into .env or a plain config file.':
    '它是内置在操作系统中的安全密码存储区：Windows 上是凭据管理器，macOS 上是钥匙串，许多 Linux 桌面上则是 Secret Service 或 KWallet。OpenWand 用它存储提供方密钥和 OAuth token，而不是写入 <code>.env</code> 或明文配置文件。',
  'Does OpenWand send everything on my screen?': 'OpenWand 会发送我屏幕上的所有内容吗？',
  'No. Context is controlled by caller profile and by the context chips in the intent overlay. OpenWand may inspect available sources locally for availability, token estimates, and redaction counts, but previewing a source does not send it to the model or save it as chat/memory.':
    '不会。上下文由调用器配置和意图覆盖层中的上下文标签控制。OpenWand 可能会在本地检查可用来源，用于显示可用性、token 估算和脱敏计数，但预览某个来源不会把它发送给模型，也不会保存为聊天或记忆。',
  ...
  'Use the portable package for your OS: Windows .exe, macOS app or launcher, or Linux portable build or launcher. If you are running from the repo, use Start OpenWand.bat, Start OpenWand.command, or Start OpenWand.sh; the first source run installs dependencies, and later runs just launch the app.':
    '请使用适合你操作系统的便携包：Windows 的 <code>.exe</code>、macOS 应用或启动器，或 Linux 便携版或启动器。如果你从源码版本运行，请使用 <code>Start OpenWand.bat</code>、<code>Start OpenWand.command</code> 或 <code>Start OpenWand.sh</code>；第一次会安装依赖，之后只会启动应用。',
  ...
  'Yes, if they expose an OpenAI-compatible endpoint. Ollama works through its /v1 endpoint, and LM Studio / vLLM can be used through the custom endpoint route. OpenWand does not directly speak native, non-OpenAI-compatible local model APIs.':
    '可以，只要它们提供 OpenAI 兼容端点即可。Ollama 通过它的 <code>/v1</code> 端点工作，LM Studio / vLLM 可通过自定义端点路由使用。OpenWand 目前不会直接调用原生、非 OpenAI 兼容的本地模型 API。',
  ...
  'Yes. Set a primary route and optional fallback routes so OpenWand can switch when a provider is unavailable or limited.':
    '可以。设置一条主路线和可选的回退路线，这样当某个提供方不可用或受限时，OpenWand 可以切换。',
  ...
  'Provider capabilities differ. OpenWand shows model warnings when the selected route does not support a feature needed by the current request.':
    '不同提供方的能力不同。当所选路线不支持当前使求所需功能时，OpenWand 会显示模型警告。',
  ...
  'Is OpenWand free?': 'OpenWand 是免费的吗？',
  'Yes. OpenWand is free and open source. You may still pay for any model provider, TTS provider, or hosted service you choose to connect.':
    '是。OpenWand 免费且开源。不过你选择连接的模型提供方、TTS 提供方或托管服务仍可能需要付费。',
  ...
  'OpenWand falls back to CPU automatically when CUDA is not compatible.':
    '当 CUDA 不兼容时，OpenWand 会自动回退到 CPU。',
  'Details': '详细信息',
  ...
  'The log says Kokoro model failed on cuda: cuDNN version 91900 is not compatible with devices with SM < 7.5, then OpenWand falls back to CPU.':
    '日志会显示 Kokoro 模型在 <code>cuda</code> 上失败：cuDNN 版本 91900 与 SM 低于 7.5 的设备不兼容，随后 OpenWand 会回退到 CPU。',
  ...
  'OpenWand already falls back safely. We may add a preflight CUDA capability check so unsupported GPUs skip the failed CUDA attempt.':
    'OpenWand 已经会安全回退。之后可能会加入 CUDA 能力预检查，让不受支持的 GPU 跳过注定失败的 CUDA 尝试。',
  'Start here': '从这里开始',
  ...
  'Install Python 3.12, then rerun Start OpenWand.command. macOS installs from requirements/requirements-macos.lock.':
    '安装 Python <code>3.12</code>，然后重新运行 <code>Start OpenWand.command</code>。macOS 会从 <code>requirements/requirements-macos.lock</code> 安装。',
  ...
  'Enable Browser/Web context for the caller. If the chip says deferred, OpenWand may fetch page text only after you send.':
    '为调用器启用浏览器/网页上下文。如果标签显示延迟，OpenWand 可能只会在发送后获取页面文本。',
  ...
  'Grant Accessibility, Screen Recording, and Microphone permissions as needed, then restart OpenWand.':
    '根据需要授予辅助功能、屏幕录制和麦克风权限，然后重启 OpenWand。',
  'Linux': 'Linux',
...
Object.assign(I18N.reg['zh-Hans'].tr, {
  ...
  'The core OpenWand loop: press the hotkey, choose an intent, send selected or enabled context, and read the streamed answer without leaving the active app.':
    'OpenWand 的核心流程：按下热键，选择意图，发送选中或已启用的上下文，然后在不离开当前应用的情况下阅读流式回答。',
  'Vision snip': '视觉截图',
  'When visual context matters, draw a region with Ctrl Alt Q. OpenWand sends only that crop to a vision-capable model and keeps the response in the overlay.':
    '当视觉上下文很重要时，用 <kbd>Ctrl Alt Q</kbd> 框选一个区域。OpenWand 只会把这块截图发送给支持视觉的模型，并把回答保留在悬浮层中。',
  ...
  'OpenWand can gather useful app context without taking a screenshot, so the model knows what you are working on. Then the rewrite hotkey rewrites only the selected text and targets paste-back at the original field captured when you pressed the hotkey.':
    '这个演示展示两个不同功能。首先，OpenWand 可以在不截图的情况下收集有用的应用上下文，让模型知道你正在处理什么。然后，改写热键只重写选中的文本，并把回贴目标指向按下热键时捕获的原始字段。',
  ...
  'OpenWand hotkey overlay query demo': 'OpenWand 快捷键悬浮层查询演示',
  'OpenWand screen snip demo': 'OpenWand 屏幕截图演示',
  'OpenWand context-aware rewrite demo': 'OpenWand 上下文感知重写演示',
  'OpenWand multi-agent task demo': 'OpenWand 多智能体任务演示',
  'Check Settings': '检查设置',
...
Object.assign(I18N.reg['zh-Hans'].tr, {
  ...
  "Each caller has a context grid, not a single three-toggle block. These defaults decide what OpenWand may attach before the model answers, and what the model may fetch on demand during the turn.": "每个调用器都有一个上下文网格，而不是单一的三开关块。这些默认值决定了 OpenWand 在模型回答前可以附加什么，以及模型在本轮中可以按需获取什么。",
  "Control": "控制项",
  "Modes": "模式",
  ...
  "On usually means OpenWand gathers that source before sending the prompt. Let model decide exposes a tool instead, so the model can fetch the source only if the answer needs it. More context can improve answers, but it may add local parsing work, token usage, network calls, or privacy warnings depending on the source.": "<strong>开</strong>通常表示 OpenWand 在发送提示词前收集该来源。<strong>由模型决定</strong>则改为暴露一个工具，使模型仅在回答需要时才获取该来源。更多上下文可以改善回答，但根据来源不同，可能增加本地解析工作、token 用量、网络调用或隐私警告。",
...
Object.assign(I18N.reg['zh-Hans'].tr, {
  ...
  "Open-source coding and agent workflows, especially when GLM is exposed through an API route OpenWand can call.": "开源编程与智能体工作流，尤其是当 GLM 通过 OpenWand 能调用的 API 路由暴露时。",
  ...
  "One token for many free backends; point OpenWand's custom endpoint at your local deployment.": "一个 token 对应多个免费后端；将 OpenWand 的自定义端点指向你的本地部署。",
  ...
  "One local endpoint for many backends; point OpenWand's custom endpoint at OmniRoute and use a model such as auto.": "一个本地端点连接多个后端；将 OpenWand 的自定义端点指向 OmniRoute，并使用例如 <code>auto</code> 的模型。",
  "Trial credits are useful for evaluating a model before paying, but they are usually spend-limited or time-limited. Use them for comparison runs; build daily OpenWand usage on a permanent free tier, a paid key, or a local model.": "试用额度适合在付费前评估模型，但通常有消费或时间限制。用它们做对比测试；日常的 OpenWand 使用应建立在长期免费额度、付费密钥或本地模型之上。",
  ...
  "OpenWand reaches most of these through its OpenAI-compatible client. Many now have a dedicated LLM_PROVIDER value; account-specific or deployment-specific routes still work through the custom endpoint if the provider exposes an OpenAI-compatible URL. Providers without that shape are usually easiest through OpenRouter or another compatible gateway. Add the key itself in Settings → LLM, where it is stored in the OS keychain.": "OpenWand 通过其兼容 OpenAI 的客户端访问其中大多数。许多提供方现在都有专用的 <code>LLM_PROVIDER</code> 值；如果提供方暴露了兼容 OpenAI 的 URL，账户专属或部署专属的路由仍可通过 <code>custom</code> 端点工作。没有这种形式的提供方通常通过 OpenRouter 或其他兼容网关最为简便。密钥本身使在 <strong>设置 → LLM</strong> 中填写，它会保存到操作系统密钥链中。",
  ...
  "Agent-specific offers such as FreeBuff's free GLM access are not automatically OpenWand API providers. OpenWand needs an API key, a compatible gateway, or a local OpenAI-compatible server.": "面向智能体的专属优惠（例如 FreeBuff 的免费 GLM 访问）并不会自动成为 OpenWand 的 API 提供方。OpenWand 需要 API 密钥、兼容网关或本地兼容 OpenAI 的服务器。",
...
Object.assign(I18N.reg['zh-Hans'].tr, {
  ...
  "OpenWand offers three mutually exclusive settings under Settings → App → Privacy mode:": "OpenWand 在<strong>设置 → 应用 → 隐私模式</strong>下提供三个互斥选项：",
  "Mode": "模式",
  "What it does": "功能",
