...
Object.assign(I18N.reg['es'].tr, {
  ...
  'Using a free source in OpenWand': 'Usar una fuente gratuita en OpenWand',
  ...
  "OpenWand is free, but it still needs a model provider to answer your queries. You don't have to begin with a paid API key — several providers offer a genuinely free tier, free monthly credits, or no-cost rate-limited access. This page rounds up the current options and shows how to connect each one to OpenWand.":
    "OpenWand es gratuito, pero aun así necesita un proveedor de modelos para responder tus consultas. No tienes que empezar con una clave de API de pago: varios proveedores ofrecen un nivel realmente gratuito, créditos mensuales gratis o acceso sin coste limitado por tasa. Esta página reúne las opciones actuales y muestra cómo conectar cada una a OpenWand.",
  ...
  "OpenWand reaches most of these through its OpenAI-compatible client. A few have a dedicated LLM_PROVIDER value; everything else works through the custom endpoint by pointing CUSTOM_BASE_URL at the provider's OpenAI-compatible URL. Add the key itself in Settings → LLM, where it is stored in the OS keychain.":
    "OpenWand llega a la mayoría de ellos a través de su cliente compatible con OpenAI. Unos pocos tienen un valor <code>LLM_PROVIDER</code> dedicado; todo lo demás funciona mediante el endpoint <code>custom</code> apuntando <code>CUSTOM_BASE_URL</code> a la URL compatible con OpenAI del proveedor. Añade la clave en <strong>Ajustes → LLM</strong>, donde se guarda en el llavero del sistema.",
  'If you run a model on your own machine there are no tokens to bill and nothing leaves the device. Ollama, LM Studio, and vLLM all expose an OpenAI-compatible server that OpenWand talks to through the custom provider.':
    "Si ejecutas un modelo en tu propia máquina no hay tokens que facturar y nada sale del dispositivo. <strong>Ollama</strong>, <strong>LM Studio</strong> y <strong>vLLM</strong> exponen un servidor compatible con OpenAI con el que OpenWand se comunica mediante el proveedor <code>custom</code>.",
  ...
  'Front-end browser SDK only — it is not a backend API OpenWand can call.':
    "Solo SDK de navegador en el front-end: no es una API de backend que OpenWand pueda llamar.",
  ...
  "Some free tiers may use your prompts to improve their models — don't send sensitive context to them. OpenWand's redaction still applies either way.":
    "Algunos niveles gratuitos pueden usar tus prompts para mejorar sus modelos: no les envíes contexto sensible. La <a onclick=\"navigate('security')\">redacción</a> de OpenWand se aplica igualmente.",
  ...
  "Puter.js is a browser SDK, not a server API, so it can't be set as a OpenWand LLM_PROVIDER.":
    "Puter.js es un SDK de navegador, no una API de servidor, así que no puede usarse como <code>LLM_PROVIDER</code> de OpenWand.",

  /* Overview — "What you get" */
  'What you get': 'Lo que obtienes',
  'OpenWand lives as a small animated icon in the corner of your screen — always on top, never in your way. Press the hotkey and a quick picker drops in; choose an action or type your own, and OpenWand grabs the right context, streams the reply, and can read it aloud word by word.':
    "OpenWand vive como un pequeño icono animado en una esquina de tu pantalla — siempre encima, nunca en tu camino. Pulsa el atajo y aparece un selector rápido; elige una acción o escribe la tuya, y OpenWand toma el contexto adecuado, responde en aproximadamente un segundo y medio y lee la respuesta en voz alta, palabra por palabra.",
  'Any app': 'Cualquier app',
  'Ask from anywhere': 'Pregunta desde cualquier lugar',
  'OpenWand listens for your custom hotkey across apps, opens with minimal prompt delay, and sends the selected context without a mouse or window switch.':
    "OpenWand escucha tu atajo personalizado en todas las apps, abre el prompt con una demora mínima y envía el contexto seleccionado sin ratón ni cambio de ventana.",
  ...
  'When CALLER_N_PASTE_BACK=True, OpenWand pastes the reply straight into whichever input had focus before the overlay opened — replacing the selected text.':
    "Cuando <code>CALLER_N_PASTE_BACK=True</code>, OpenWand pega la respuesta directamente en el campo que tenía el foco antes de abrirse la superposición — reemplazando el texto seleccionado.",
  ...
  'OpenWand uses pynput (no admin rights) for caller hotkeys. If a hotkey is already claimed by Windows or another app, OpenWand will not intercept it reliably. Choose combinations that are not globally reserved.':
    "OpenWand usa <code>pynput</code> (sin derechos de administrador) para los atajos de invocador. Si un atajo ya lo reclama Windows u otra aplicación, OpenWand no lo interceptará de forma fiable. Elige combinaciones que no estén reservadas globalmente.",
  ...
  /* Bubble appearance */
  'Bubble': 'Burbuja',
  'The reply bubble is a transparent, always-on-top Qt window owned by the openwand-ui worker. Visual properties can be edited in Settings; source checkouts can also edit the same values in .env:':
    "La burbuja de respuesta es una ventana Qt transparente y siempre encima, propiedad del worker <code>openwand-ui</code>. Las propiedades visuales se pueden editar en Ajustes; las copias desde código fuente también pueden editar los mismos valores en <code>.env</code>:",
  ...
  /* Provider: Groq */
  'Groq exposes an OpenAI-compatible API so OpenWand uses the openai Python package to talk to it. It is a good choice for latency-sensitive hotkey queries thanks to its low time-to-first-token.':
    "Groq expone una API compatible con OpenAI, por lo que OpenWand usa el paquete de Python <code>openai</code> para comunicarse con él. Es una buena opción para consultas por atajo sensibles a la latencia gracias a su bajo tiempo hasta el primer token.",
  ...
  'This route is stable today, but it depends on OpenAI continuing to allow subscription-backed OAuth access from third-party clients. Provider policy can change later, so keep an API-key, local, or other provider route as a fallback if OpenWand is part of your daily workflow.':
    'Esta ruta es estable hoy, pero depende de que OpenAI siga permitiendo acceso OAuth respaldado por suscripción desde clientes de terceros. La política del proveedor puede cambiar más adelante, así que mantén una ruta de respaldo con clave API, local u otro proveedor si OpenWand forma parte de tu flujo diario.',
  ...
  'Model availability depends on your subscription and what the OAuth route exposes to OpenWand. Start with the default shown in Settings, then adjust only if the selected model is available on your account.':
    'La disponibilidad de modelos depende de tu suscripción y de lo que la ruta OAuth exponga a OpenWand. Empieza con el valor predeterminado que se muestra en Ajustes y cambia solo si el modelo elegido está disponible en tu cuenta.',
  ...
  'OpenWand uses the openai Python package for all OpenAI-compatible endpoints. The following providers work by setting the right LLM_PROVIDER value and adding the API key in Settings:':
    "OpenWand usa el paquete de Python <code>openai</code> para todos los puntos finales compatibles con OpenAI. Los siguientes proveedores funcionan estableciendo el valor correcto de <code>LLM_PROVIDER</code> y añadiendo la clave de API en Ajustes:",
  ...
  'Antivirus': 'Antivirus',
  'Some antivirus products flag keyboard hooks. You may need to add the app directory or OpenWand.exe to your AV exclusion list.':
    "Algunos antivirus marcan los hooks de <code>keyboard</code>. Es posible que debas añadir el directorio de la aplicación o <code>OpenWand.exe</code> a la lista de exclusiones de tu antivirus.",
  ...
  /* Platform: macOS */
  'OpenWand runs natively on macOS 13 (Ventura) and later, on both Apple Silicon and Intel Macs. The overlay, voice, context capture, and memory are all supported.':
    "OpenWand se ejecuta de forma nativa en macOS 13 (Ventura) y posteriores, tanto en Mac con Apple Silicon como con Intel. La superposición, la voz, la captura de contexto y la memoria son todas compatibles.",
  'macOS packaged build status': 'Estado del paquete de macOS',
  'The packaged macOS build was last live-tested quite a while ago, so it may be buggier than the Windows build or the repo launcher path. If it gives you trouble, please try the repo version with Start OpenWand.command; it is the best-supported macOS path right now. Renting Apple hardware for fresh testing costs money, so if you would like to support more macOS verification, you can donate at Buy Me a Coffee. No pressure either way: clear bug reports with logs are also very helpful.':
    "El build empaquetado de macOS se probó en vivo por última vez hace bastante tiempo, así que puede tener más errores que el build de Windows o la ruta del lanzador desde el repositorio. Si te da problemas, prueba la versión del repositorio con <code>Start OpenWand.command</code>; ahora mismo es la ruta de macOS mejor respaldada. Alquilar hardware de Apple para hacer pruebas nuevas cuesta dinero, así que si quieres apoyar más verificación de macOS, puedes donar en <a href=\"https://buymeacoffee.com/sunnylich\" target=\"_blank\">Buy Me a Coffee</a>. Sin presión: los informes de bugs claros con registros también ayudan mucho.",
  'Area': 'Área',
  ...
  'macOS gates input and screen APIs behind the privacy system (TCC). On first run, grant OpenWand the following under System Settings → Privacy & Security:':
    "macOS protege las API de entrada y pantalla tras el sistema de privacidad (TCC). En la primera ejecución, concede a OpenWand lo siguiente en <strong>Ajustes del Sistema → Privacidad y seguridad</strong>:",
  ...
  'Input Monitoring — required for the global hotkey listener (a purpose-built PyObjC/Carbon backend in openwand-native)':
    "<strong>Monitorización de entrada</strong> — necesaria para el detector de atajos globales (un backend PyObjC/Carbon específico en <code>openwand-native</code>)",
  ...
  'macOS only applies new Accessibility / Input Monitoring grants to a process after it is relaunched. Quit and reopen OpenWand once permissions are checked.':
    "macOS solo aplica los nuevos permisos de Accesibilidad / Monitorización de entrada a un proceso después de relanzarlo. Cierra y vuelve a abrir OpenWand una vez marcados los permisos.",
  ...
  'For a session that keeps full runtime logs, start OpenWand with Start OpenWand Debug.command instead of the normal launcher.':
    "Para una sesión que conserve los registros de ejecución completos, inicia OpenWand con <code>Start OpenWand Debug.command</code> en lugar del lanzador normal.",
  ...
  'OpenWand is best supported on X11 sessions today. We are currently working on Linux Wayland support; native hotkey, clipboard, and screen capture behavior still depends on the desktop environment.':
    'OpenWand tiene mejor soporte hoy en sesiones X11. Actualmente estamos trabajando en el soporte de Linux Wayland; el comportamiento nativo de atajos, portapapeles y captura de pantalla todavía depende del entorno de escritorio.',
  ...
  /* Add-ons */
  'Add-ons are the supported way to extend OpenWand. An add-on can observe or modify query context, observe responses, contribute tray actions, expose settings, register model-callable tools, and declare its own intents and hotkeys.':
    "Los complementos son la forma admitida de ampliar OpenWand. Un complemento puede observar o modificar el contexto de la consulta, observar las respuestas, aportar acciones de bandeja, exponer ajustes, registrar herramientas invocables por el modelo y declarar sus propias intenciones y atajos.",
  ...
  'Each enabled add-on runs in its own Python host process — one process per add-on. A crash, import failure, or slow hook is isolated from the brain worker and from every other add-on. OpenWand talks to each host over a small newline-delimited JSON IPC protocol.':
    "Cada complemento habilitado se ejecuta en su <strong>propio proceso anfitrión de Python</strong> — un proceso por complemento. Un fallo, un error de importación o un hook lento queda aislado del worker «cerebro» y de todos los demás complementos. OpenWand se comunica con cada anfitrión mediante un pequeño protocolo IPC de JSON delimitado por saltos de línea.",
  'Layout': 'Estructura',
  ...
  'Capabilities are opt-in — missing permissions are denied. An add-on without tools = true can\'t register tools; one without ui = ["tray"] can\'t add tray actions. LLM actions require llm = true and are capped by OpenWand before any provider credentials are used.':
    "Las capacidades son opcionales — <strong>los permisos ausentes se deniegan</strong>. Un complemento sin <code>tools = true</code> no puede registrar herramientas; uno sin <code>ui = [\"tray\"]</code> no puede añadir acciones de bandeja. Las acciones de LLM requieren <code>llm = true</code> y OpenWand las limita antes de usar cualquier credencial de proveedor.",
  ...
  '[dependencies] is optional. Add-ons without it run from OpenWand\'s own Python runtime. Add-ons that declare packages get a dedicated virtual environment under addon_envs/<id>/; the Addon Manager shows the required packages and offers an Install/Repair action.':
    "<code>[dependencies]</code> es opcional. Los complementos sin esta sección se ejecutan desde el propio runtime de Python de OpenWand. Los complementos que declaran paquetes obtienen un entorno virtual dedicado en <code>addon_envs/&lt;id&gt;/</code>; el Gestor de complementos muestra los paquetes requeridos y ofrece una acción de Instalar/Reparar.",
  'Approval per dependency hash': 'Aprobación por hash de dependencias',
  'OpenWand records approval for the exact dependency set, so an update that changes packages must be approved again before it runs. uv is used when available, falling back to python -m venv in source checkouts.':
    "OpenWand registra la aprobación para el conjunto exacto de dependencias, de modo que una actualización que cambie los paquetes debe aprobarse de nuevo antes de ejecutarse. Se usa <code>uv</code> cuando está disponible, recurriendo a <code>python -m venv</code> en las copias de código fuente.",
  ...
  'Distribution is supported with .zip or .openwand archives containing one add-on folder; the Addon Manager can also install from an unpacked folder.':
    "La distribución se admite con archivos <code>.zip</code> o <code>.openwand</code> que contengan una carpeta de complemento; el Gestor de complementos también puede instalar desde una carpeta descomprimida.",
  ...
  /* Tool plugins */
  'Legacy': 'Heredado',
  'Script tools in tools/installed/ still load, but the supported way to extend OpenWand is now Add-ons — they run in isolated processes and do far more than register a tool.':
    "Las herramientas de script en <code>tools/installed/</code> siguen cargándose, pero la forma admitida de ampliar OpenWand son ahora los <a onclick=\"navigate('addons')\">complementos</a> — se ejecutan en procesos aislados y hacen mucho más que registrar una herramienta.",
  ...
  'Security': 'Seguridad',
  'Tool plugins run in the same process as OpenWand with full OS access. Only install plugins you trust.':
    "Los complementos de herramientas se ejecutan en el mismo proceso que OpenWand con acceso completo al sistema. Instala solo complementos de confianza.",
  ...
  "Define at least one LLM_FALLBACKS route so a single provider outage or rate limit doesn't break your hotkeys — OpenWand tries each route in order.":
    "Define al menos una ruta <code>LLM_FALLBACKS</code> para que la caída de un solo proveedor o un límite de uso no rompa tus atajos — OpenWand prueba cada ruta en orden.",
  ...
  'Keep the contents of dist/OpenWand/ together when moving the portable build to another folder or machine.':
    "Mantén junto el contenido de <code>dist/OpenWand/</code> al mover el build portable a otra carpeta o máquina.",
  ...
  'On your machine. Settings, chats, memory, privacy reports, and local configuration are written to local app data paths, not to a OpenWand-hosted account.':
    'En tu equipo. Los ajustes, chats, memoria, informes de privacidad y configuración local se escriben en rutas de datos locales de la app, no en una cuenta hospedada por OpenWand.',
  'What is the OS keychain?': '¿Qué es el llavero del sistema?',
  'It is the secure password store built into your operating system: Windows Credential Manager on Windows, Keychain on macOS, and Secret Service or KWallet on many Linux desktops. OpenWand uses it for provider keys and OAuth tokens instead of writing them into .env or a plain config file.':
    'Es el almacén seguro de contraseñas integrado en tu sistema operativo: Administrador de credenciales en Windows, Llavero en macOS, y Secret Service o KWallet en muchos escritorios Linux. OpenWand lo usa para claves de proveedor y tokens OAuth en vez de escribirlos en <code>.env</code> o en un archivo de configuración en texto plano.',
  'Does OpenWand send everything on my screen?': '¿OpenWand envía todo lo que hay en mi pantalla?',
  'No. Context is controlled by caller profile and by the context chips in the intent overlay. OpenWand may inspect available sources locally for availability, token estimates, and redaction counts, but previewing a source does not send it to the model or save it as chat/memory.':
    'No. El contexto se controla mediante el perfil de invocador y las fichas de contexto en la superposición de intención. OpenWand puede inspeccionar fuentes disponibles localmente para mostrar disponibilidad, estimaciones de tokens y recuentos de censura, pero previsualizar una fuente no la envía al modelo ni la guarda como chat/memoria.',
  ...
  'Use the portable package for your OS: Windows .exe, macOS app or launcher, or Linux portable build or launcher. If you are running from the repo, use Start OpenWand.bat, Start OpenWand.command, or Start OpenWand.sh; the first source run installs dependencies, and later runs just launch the app.':
    'Usa la app empaquetada o el build portable para tu sistema: el <code>.exe</code> de Windows, la app o lanzador de macOS, o el build portable o lanzador de Linux. Si ejecutas desde el repositorio, usa <code>Start OpenWand.bat</code>, <code>Start OpenWand.command</code> o <code>Start OpenWand.sh</code>; la primera ejecución desde código fuente instala dependencias y las siguientes solo inician la app.',
  ...
  'Yes, if they expose an OpenAI-compatible endpoint. Ollama works through its /v1 endpoint, and LM Studio / vLLM can be used through the custom endpoint route. OpenWand does not directly speak native, non-OpenAI-compatible local model APIs.':
    'Sí, si exponen un endpoint compatible con OpenAI. Ollama funciona mediante su endpoint <code>/v1</code>, y LM Studio / vLLM pueden usarse mediante la ruta de endpoint personalizado. OpenWand no habla directamente con APIs locales nativas que no sean compatibles con OpenAI.',
  'Can I use more than one provider?': '¿Puedo usar más de un proveedor?',
  'Yes. Set a primary route and optional fallback routes so OpenWand can switch when a provider is unavailable or limited.':
    'Sí. Define una ruta principal y rutas de respaldo opcionales para que OpenWand pueda cambiar cuando un proveedor no esté disponible o esté limitado.',
  ...
  'Provider capabilities differ. OpenWand shows model warnings when the selected route does not support a feature needed by the current request.':
    'Las capacidades varían según el proveedor. OpenWand muestra advertencias cuando la ruta seleccionada no admite una función necesaria para la solicitud actual.',
  ...
  'Is OpenWand free?': '¿OpenWand es gratis?',
  'Yes. OpenWand is free and open source. You may still pay for any model provider, TTS provider, or hosted service you choose to connect.':
    'Sí. OpenWand es gratis y de código abierto. Aun así, puedes pagar por cualquier proveedor de modelos, proveedor TTS o servicio alojado que decidas conectar.',
  ...
  'OpenWand falls back to CPU automatically when CUDA is not compatible.':
    'OpenWand vuelve automáticamente a CPU cuando CUDA no es compatible.',
  'Details': 'Detalles',
  'What users see': 'Qué ven los usuarios',
  'The log says Kokoro model failed on cuda: cuDNN version 91900 is not compatible with devices with SM < 7.5, then OpenWand falls back to CPU.':
    'El registro dice que el modelo Kokoro falló en <code>cuda</code>: la versión cuDNN 91900 no es compatible con dispositivos con SM inferior a 7.5, y luego OpenWand vuelve a CPU.',
  ...
  'OpenWand already falls back safely. We may add a preflight CUDA capability check so unsupported GPUs skip the failed CUDA attempt.':
    'OpenWand ya vuelve a CPU de forma segura. Podemos añadir una comprobación previa de capacidad CUDA para que las GPU no compatibles omitan el intento CUDA que fallaría.',
  ...
  'Install Python 3.12, then rerun Start OpenWand.command. macOS installs from requirements/requirements-macos.lock.':
    'Instala Python <code>3.12</code> y vuelve a ejecutar <code>Start OpenWand.command</code>. macOS instala desde <code>requirements/requirements-macos.lock</code>.',
  ...
  'Enable Browser/Web context for the caller. If the chip says deferred, OpenWand may fetch page text only after you send.':
    'Activa el contexto Navegador/Web para el invocador. Si la ficha dice diferido, OpenWand puede obtener el texto de la página solo después de enviar.',
  ...
  'Grant Accessibility, Screen Recording, and Microphone permissions as needed, then restart OpenWand.':
    'Concede Accesibilidad, Grabación de pantalla y Micrófono según sea necesario, y reinicia OpenWand.',
  'Linux': 'Linux',
...
Object.assign(I18N.reg['es'].tr, {
  ...
  'The core OpenWand loop: press the hotkey, choose an intent, send selected or enabled context, and read the streamed answer without leaving the active app.':
    'El ciclo central de OpenWand: pulsa el atajo, elige una intención, envía el contexto seleccionado o activado y lee la respuesta en streaming sin salir de la app activa.',
  'Vision snip': 'Recorte visual',
  'When visual context matters, draw a region with Ctrl Alt Q. OpenWand sends only that crop to a vision-capable model and keeps the response in the overlay.':
    'Cuando importa el contexto visual, dibuja una región con <kbd>Ctrl Alt Q</kbd>. OpenWand envía solo ese recorte a un modelo con visión y mantiene la respuesta en la superposición.',
  'Context-aware rewrite': 'Reescritura con contexto',
  'OpenWand can gather useful app context without taking a screenshot, so the model knows what you are working on. Then the rewrite hotkey rewrites only the selected text and targets paste-back at the original field captured when you pressed the hotkey.':
    'Esta demo muestra dos funciones distintas. Primero, OpenWand puede recopilar contexto util de la app sin hacer una captura de pantalla, para que el modelo sepa en que estas trabajando. Luego, el atajo de reescritura reescribe solo el texto seleccionado y dirige el pegado de vuelta al campo original capturado cuando pulsaste el atajo.',
  ...
  'OpenWand hotkey overlay query demo': 'Demo de consulta de OpenWand con atajo en la superposición',
  'OpenWand screen snip demo': 'Demo de recorte de pantalla de OpenWand',
  'OpenWand context-aware rewrite demo': 'Demo de reescritura con contexto de OpenWand',
  'OpenWand multi-agent task demo': 'Demo de tarea multiagente de OpenWand',
...
Object.assign(I18N.reg['es'].tr, {
  ...
  "OpenWand ships with an MCP bridge add-on (addons/mcp_bridge). List any Model Context Protocol servers in its servers.json and it connects to each one and exposes their whole toolkit to the model as OpenWand tools — so any MCP server becomes callable from the overlay. It includes a small example_server.py you can point it at to try it out. Read addons/README.md for the full add-on contract.": "OpenWand incluye un complemento <strong>MCP bridge</strong> (<code>addons/mcp_bridge</code>). Enumera cualquier servidor <a href=\"https://modelcontextprotocol.io\" target=\"_blank\" rel=\"noopener\">Model Context Protocol</a> en su <code>servers.json</code> y se conecta a cada uno, exponiendo todo su conjunto de herramientas al modelo como herramientas de OpenWand — de modo que cualquier servidor MCP se puede invocar desde la superposición. Incluye un pequeño <code>example_server.py</code> al que puedes apuntarlo para probarlo. Lee <code>addons/README.md</code> para conocer el contrato completo de complementos.",
  "OpenWand is free, but it still needs a model provider to answer your queries. You don't have to begin with a paid API key — several providers offer free-tier examples, free monthly credits, or no-cost rate-limited access. This page shows examples of providers you can connect in OpenWand.": "OpenWand es gratis, pero aun así necesita un proveedor de modelos para responder a tus consultas. No tienes que empezar con una clave de API de pago — varios proveedores ofrecen ejemplos de planes gratuitos, créditos mensuales gratuitos o acceso sin coste con límites de velocidad. Esta página muestra ejemplos de proveedores que puedes conectar en OpenWand.",
  ...
  "Each caller has a context grid, not a single three-toggle block. These defaults decide what OpenWand may attach before the model answers, and what the model may fetch on demand during the turn.": "Cada invocador tiene una cuadrícula de contexto, no un único bloque de tres conmutadores. Estos valores predeterminados deciden qué puede adjuntar OpenWand antes de que el modelo responda, y qué puede obtener el modelo bajo demanda durante el turno.",
  "Control": "Control",
  "Modes": "Modos",
  ...
  "On usually means OpenWand gathers that source before sending the prompt. Let model decide exposes a tool instead, so the model can fetch the source only if the answer needs it. More context can improve answers, but it may add local parsing work, token usage, network calls, or privacy warnings depending on the source.": "<strong>Activado</strong> normalmente significa que OpenWand reúne esa fuente antes de enviar el prompt. <strong>Dejar decidir al modelo</strong> expone en su lugar una herramienta, de modo que el modelo solo obtiene la fuente si la respuesta la necesita. Más contexto puede mejorar las respuestas, pero puede añadir trabajo de análisis local, consumo de tokens, llamadas de red o avisos de privacidad según la fuente.",
...
Object.assign(I18N.reg['es'].tr, {
  ...
  "Open-source coding and agent workflows, especially when GLM is exposed through an API route OpenWand can call.": "Flujos de trabajo de programación y de agentes de código abierto, sobre todo cuando GLM se expone a través de una ruta de API que OpenWand puede llamar.",
  ...
  "One token for many free backends; point OpenWand's custom endpoint at your local deployment.": "Un solo token para muchos backends gratuitos; apunta el endpoint personalizado de OpenWand a tu despliegue local.",
  ...
  "One local endpoint for many backends; point OpenWand's custom endpoint at OmniRoute and use a model such as auto.": "Un endpoint local para muchos backends; apunta el endpoint personalizado de OpenWand a OmniRoute y usa un modelo como <code>auto</code>.",
  "Local — Ollama / LM Studio / vLLM": "Local — Ollama / LM Studio / vLLM",
  "Trial credits are useful for evaluating a model before paying, but they are usually spend-limited or time-limited. Use them for comparison runs; build daily OpenWand usage on a permanent free tier, a paid key, or a local model.": "Los créditos de prueba son útiles para evaluar un modelo antes de pagar, pero suelen tener un límite de gasto o de tiempo. Úsalos para ejecuciones comparativas; construye tu uso diario de OpenWand sobre un nivel gratuito permanente, una clave de pago o un modelo local.",
  ...
  "OpenWand reaches most of these through its OpenAI-compatible client. Many now have a dedicated LLM_PROVIDER value; account-specific or deployment-specific routes still work through the custom endpoint if the provider exposes an OpenAI-compatible URL. Providers without that shape are usually easiest through OpenRouter or another compatible gateway. Add the key itself in Settings → LLM, where it is stored in the OS keychain.": "OpenWand llega a la mayoría de ellos a través de su cliente compatible con OpenAI. Muchos ya tienen un valor <code>LLM_PROVIDER</code> dedicado; las rutas específicas de cuenta o de despliegue siguen funcionando mediante el endpoint <code>custom</code> si el proveedor expone una URL compatible con OpenAI. Los proveedores sin esa forma suelen ser más fáciles a través de OpenRouter u otra pasarela compatible. Añade la clave en <strong>Ajustes → LLM</strong>, donde se guarda en el llavero del sistema.",
  ...
  "Agent-specific offers such as FreeBuff's free GLM access are not automatically OpenWand API providers. OpenWand needs an API key, a compatible gateway, or a local OpenAI-compatible server.": "Las ofertas específicas para agentes, como el acceso gratuito a GLM de FreeBuff, no son automáticamente proveedores de API para OpenWand. OpenWand necesita una clave de API, una pasarela compatible o un servidor local compatible con OpenAI.",
...
Object.assign(I18N.reg['es'].tr, {
  "Sensitive text filtered before cloud sends": "Texto sensible filtrado antes de enviarlo a la nube",
  "Built-in patterns — and optional local AI — replace likely sensitive text before cloud-bound requests are sent.": "Los patrones integrados —y la IA local opcional— sustituyen el texto probablemente sensible antes de enviar solicitudes a la nube.",
  "Choose your privacy mode": "Elige tu modo de privacidad",
  "OpenWand offers three mutually exclusive settings under Settings → App → Privacy mode:": "OpenWand ofrece tres opciones mutuamente excluyentes en <strong>Configuración → Aplicación → Modo de privacidad</strong>:",
  "Mode": "Modo",
  "What it does": "Qué hace",
  "Off": "<strong>Desactivado</strong>",
  "No automatic privacy filtering.": "Sin filtrado automático de privacidad.",
  "Built-in (default)": "<strong>Integrado</strong> (predeterminado)",
  "Local pattern detection for credentials, tokens, payment details, and other structured secrets.": "Detección local mediante patrones de credenciales, tokens, datos de pago y otros secretos estructurados.",
  "Advanced": "<strong>Avanzado</strong>",
  "The built-in detector plus a local, context-aware AI model for names, addresses, email addresses, phone numbers, private URLs and dates, account numbers, and secrets.": "El detector integrado junto con un modelo de IA local y sensible al contexto para nombres, direcciones, direcciones de correo electrónico, números de teléfono, URL y fechas privadas, números de cuenta y secretos.",
  "Advanced privacy runs locally": "La privacidad avanzada se ejecuta localmente",
...
Object.assign(I18N.reg['es'].nav.labels, {
  "ChatGPT, Codex & Claude CLI": "ChatGPT, Codex y Claude CLI",
});
Object.assign(I18N.reg['es'].meta, {
  "live-agents": {
    title: "ChatGPT, Codex y Claude CLI",
    sub: "Ejecuta sesiones completas de agentes locales detrás de OpenWand, con progreso en vivo, aprobaciones y continuidad reanudable.",
  },
});
Object.assign(I18N.reg['es'].tr, {
  "OpenWand": "OpenWand",
  ...
  "Visible reasoning": "Razonamiento visible",
  "Permission mode": "Modo de permisos",
  "How the agent modes work": "Cómo funcionan los modos de agente",
  "OpenWand can use more than a model API. It can run a complete local coding-agent harness behind both the overlay and the full chat window. The ChatGPT option starts the installed Codex CLI in app-server mode; Claude Agent uses the Claude Agent SDK and the same Claude Code CLI authentication. OpenWand supplies the prompt and captured context, then streams the agent's visible output back into the normal OpenWand interface.": "OpenWand puede usar mucho más que una API de modelos. Puede ejecutar un agente de programación local completo detrás de la superposición y de la ventana de chat. La opción <strong>ChatGPT</strong> inicia la CLI de Codex instalada en modo app-server; <strong>Claude Agent</strong> usa el SDK de Claude Agent y la misma autenticación de la CLI de Claude Code. OpenWand aporta el prompt y el contexto capturado y transmite la salida visible del agente a la interfaz normal de OpenWand.",
  "Run conversations with": "Ejecutar conversaciones con",
  "Behavior": "Comportamiento",
  "Uses the LLM provider and model configured in OpenWand.": "Usa el proveedor LLM y el modelo configurados en OpenWand.",
  "Runs the local Codex CLI app-server and uses your ChatGPT/Codex account.": "Ejecuta el app-server local de la CLI de Codex y usa tu cuenta de ChatGPT/Codex.",
  "Runs the Claude Agent SDK with the Claude Code CLI and uses your Claude account.": "Ejecuta el SDK de Claude Agent con la CLI de Claude Code y usa tu cuenta de Claude.",
  "One OpenWand interface": "Una sola interfaz de OpenWand",
  "Hotkey queries and full-chat prompts use the selected engine. OpenWand still captures context, applies privacy controls, displays the stream, and stores a local copy of the conversation.": "Las consultas por atajo y los prompts del chat completo usan el motor seleccionado. OpenWand sigue capturando contexto, aplicando los controles de privacidad, mostrando la transmisión y guardando una copia local de la conversación.",
  "Select and sign in": "Seleccionar e iniciar sesión",
  "Open Settings → App → Run conversations with and choose OpenWand, ChatGPT, or Claude Agent. When an agent is selected, the same card shows its login status and provides Sign in, Sign out, and Refresh actions. Sign-in opens the provider's terminal and browser flow.": "Abre <strong>Configuración → Aplicación → Ejecutar conversaciones con</strong> y elige OpenWand, ChatGPT o Claude Agent. Al seleccionar un agente, la misma tarjeta muestra el estado de inicio de sesión y ofrece las acciones <strong>Iniciar sesión</strong>, <strong>Cerrar sesión</strong> y <strong>Actualizar</strong>. El inicio de sesión abre el flujo de terminal y navegador del proveedor.",
  "ChatGPT mode requires the Codex CLI. OpenWand gives Codex an isolated local profile, so its login and resumable OpenWand sessions do not appear in your personal Codex history. Claude Agent uses the bundled Claude Agent SDK when available and authenticates through the Claude Code CLI.": "El modo ChatGPT requiere la CLI de Codex. OpenWand proporciona a Codex un perfil local aislado, por lo que su inicio de sesión y las sesiones reanudables de OpenWand no aparecen en tu historial personal de Codex. Claude Agent usa el SDK de Claude Agent incluido cuando está disponible y se autentica mediante la CLI de Claude Code.",
  "Choose who owns continuity": "Elegir quién controla la continuidad",
  "The Conversation goes to setting decides how follow-up prompts carry context:": "El ajuste <strong>La conversación va a</strong> decide cómo conservan el contexto los prompts posteriores:",
  "Owner": "Responsable",
  "What happens": "Qué ocurre",
  "Best for": "Ideal para",
  "OpenWand sends the complete local OpenWand history on every request. Each request starts without a retained provider continuation link.": "OpenWand envía todo el historial local de OpenWand en cada solicitud. Cada solicitud empieza sin conservar un enlace de continuación del proveedor.",
  "Portable conversations that can change engines without depending on a provider session.": "Conversaciones portátiles que pueden cambiar de motor sin depender de una sesión del proveedor.",
  "ChatGPT or Claude Agent": "ChatGPT o Claude Agent",
  "OpenWand transfers the existing history once, stores the returned provider session ID, and resumes that session for later prompts.": "OpenWand transfiere una vez el historial existente, guarda el ID de sesión devuelto por el proveedor y reanuda esa sesión en los prompts posteriores.",
  "Long agent conversations that should preserve native provider state efficiently.": "Conversaciones largas con agentes que deben conservar de forma eficiente el estado nativo del proveedor.",
  "OpenWand keeps separate OpenWand, ChatGPT/Codex, and Claude conversation namespaces so switching engines does not append a prompt to the wrong provider's history. Changing the project folder also starts a clean provider session.": "OpenWand mantiene separados los espacios de conversación de OpenWand, ChatGPT/Codex y Claude para que cambiar de motor no añada un prompt al historial del proveedor equivocado. Cambiar la carpeta del proyecto también inicia una sesión limpia del proveedor.",
  "What appears while the agent works": "Qué aparece mientras trabaja el agente",
  "Replies stream normally, while visible reasoning summaries, plans, tool starts, command or file progress, status updates, and approval requests appear as progress in the bubble and chat. OpenWand can only show events the provider exposes; private hidden chain-of-thought is never available.": "Las respuestas se transmiten normalmente, mientras que los resúmenes de razonamiento visibles, planes, inicios de herramientas, progreso de comandos o archivos, estados y solicitudes de aprobación aparecen como progreso en la burbuja y el chat. OpenWand solo puede mostrar los eventos que expone el proveedor; la cadena de pensamiento privada y oculta nunca está disponible.",
  "When a command, file change, or other protected tool needs permission, OpenWand presents the action, path, command, or diff it received and sends your decision back to the running agent.": "Cuando un comando, un cambio de archivo u otra herramienta protegida necesita permiso, OpenWand muestra la acción, ruta, comando o diff recibido y devuelve tu decisión al agente en ejecución.",
  "Live provider controls": "Controles en vivo del proveedor",
  "When ChatGPT or Claude Agent is active, a provider badge appears below the floating OpenWand icon. Click it to change settings for the next turn:": "Cuando ChatGPT o Claude Agent está activo, aparece una insignia del proveedor bajo el icono flotante de OpenWand. Haz clic en ella para cambiar los ajustes del siguiente turno:",
  "Control": "Control",
  "Effect": "Efecto",
  "Use the provider default or enter a supported model name.": "Usa el modelo predeterminado del proveedor o introduce el nombre de un modelo compatible.",
  "Choose the folder the agent may inspect or modify, or leave it on Auto.": "Elige la carpeta que el agente puede inspeccionar o modificar, o déjala en Automático.",
  "Requests the provider's faster tier when supported; it may use more quota or cost more.": "Solicita el nivel rápido del proveedor cuando sea compatible; puede consumir más cuota o costar más.",
  "Sets the effort level passed to the provider when supported.": "Establece el nivel de esfuerzo enviado al proveedor cuando sea compatible.",
  "Choose detailed or concise Codex summaries, provider Claude summaries, or turn summaries off.": "Elige resúmenes detallados o concisos de Codex, resúmenes de Claude del proveedor o desactiva los resúmenes del turno.",
  "Ask for permission, allow changes inside the project, or keep the agent in plan-only read-only mode.": "Pide permiso, permite cambios dentro del proyecto o mantiene el agente en modo de solo planificación y lectura.",
  "Older provider versions that do not support a reasoning or approval option are retried with compatible settings when possible.": "Las versiones antiguas del proveedor que no admiten alguna opción de razonamiento o aprobación vuelven a intentarse con ajustes compatibles cuando es posible.",
  "Project and context behavior": "Comportamiento del proyecto y del contexto",
  "In Auto mode, OpenWand reuses the resumed session's folder, then looks at attached files and file context, and finally falls back to OpenWand's current directory. A manually selected project overrides that inference. Agent write access remains limited to the selected project, and Codex runs without network access in its workspace sandbox.": "En modo Automático, OpenWand reutiliza la carpeta de la sesión reanudada, después consulta los archivos adjuntos y el contexto de archivos y, por último, usa el directorio actual de OpenWand. Un proyecto elegido manualmente sustituye esa inferencia. El acceso de escritura del agente queda limitado al proyecto seleccionado y Codex se ejecuta sin acceso a la red en el entorno aislado del espacio de trabajo.",
  "Selected text, ambient app context, attachments, memory, and privacy-filtered content are assembled by OpenWand before the turn. Provider-specific system prompts stay in the native harness instructions instead of being pasted into your message.": "OpenWand reúne antes del turno el texto seleccionado, el contexto de la aplicación, los adjuntos, la memoria y el contenido filtrado por privacidad. Los prompts de sistema específicos del proveedor permanecen en las instrucciones nativas del agente en vez de pegarse en tu mensaje.",
  "Offline transcript fallback": "Alternativa de transcripciones sin conexión",
  "Live agent sessions are the normal path. OpenWand also provides local transcript pull, push, and export as an experimental compatibility fallback. Pull discovers Codex and Claude JSONL histories on disk without contacting either provider. Push requires confirmation, creates a full backup, and appends only OpenWand-only turns to an existing transcript. Export requires confirmation and creates a new provider transcript without overwriting existing history.": "Las sesiones de agente en vivo son la ruta normal. OpenWand también ofrece importar, devolver y exportar transcripciones locales como alternativa experimental de compatibilidad. La importación descubre historiales JSONL de Codex y Claude en el disco sin contactar con los proveedores. La devolución requiere confirmación, crea una copia de seguridad completa y añade solo los turnos exclusivos de OpenWand a una transcripción existente. La exportación requiere confirmación y crea una transcripción nueva del proveedor sin sobrescribir el historial existente.",
  "Local files, separate from live sessions": "Archivos locales, separados de las sesiones en vivo",
  "Transcript sync edits local compatibility files; it does not send a live request or replace the supported Codex app-server and Claude Agent SDK session flow.": "La sincronización de transcripciones edita archivos locales de compatibilidad; no envía una solicitud en vivo ni sustituye el flujo de sesiones compatible del app-server de Codex y el SDK de Claude Agent.",
});
