"""
openwand_brain.host — the brain worker entry point. Run as::

    python -m openwand_brain.host
...
def _bootstrap_path() -> None:
    """Make ``core`` (repo root) and ``openwand_brain`` (runtime/brain) importable
    regardless of cwd, so the brain worker runs the same whether launched in-repo for
    tests or from the bundled runtime inside OpenWand.app."""
...
def _main() -> int:
    """Handle main for runtime brain openwand brain host."""
    _bootstrap_path()

    if os.getenv("OPENWAND_BRAIN_FAULTHANDLER"):
        import faulthandler
    ...
    from openwand_brain.handlers import HANDLERS, STREAMING, StreamContext
    ...
    from openwand_brain.handlers import set_runtime_event_emitter
    ...
    def respond(req_id, ok: bool, *, result=None, error=None) -> None:
        """Handle respond for runtime brain openwand brain host."""
    ...
    try:
        from openwand_brain.handlers import run_addon_shutdown
        run_addon_shutdown()
    except Exception:
        pass
