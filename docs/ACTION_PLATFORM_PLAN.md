# OpenWand Action Platform Plan

This document is the source of truth for OpenWand's app-aware action work. Update
...
selection-focused workflow and does not show provider actions. OpenWand must show a
...
visible after three seconds, OpenWand shows a non-blocking progress notice and
...
the same document identity, OpenWand keeps the file source and suppresses the noisy
...
access. OpenWand still owns preview, approval, target revalidation, API execution,
...
| PowerPoint web | Create/restyle options are visible but disabled with a connection explanation | Typed Office.js bridge protocol, snapshots, previews, validation, verification, and rollback contracts; fake-client tests | Not usable until OpenWand's authenticated Office.js bridge exists |
...
| Excel desktop | No production picker action yet | Real hidden `DispatchEx` acceptance created and verified `OpenWandTable` plus a six-value `OpenWandChart`, preserved source cells, and kept focus unchanged | Object-model adapter verified; picker/shared-runner route and complete rollback remain |
...
Gmail filters have no disabled state in the Gmail API, so OpenWand refuses that
...
one replace-in-place bubble line. The public stages identify the real work OpenWand
...
The first bounded action set works end to end for a OpenWand-managed LibreOffice
Calc session:
...
- The General caller detects the exact Calc window before opening the intent overlay,
  ...
  popup is visible. OpenWand gathers the selected range, values, and fingerprint
...
- The action uses LibreOffice's UNO API in the same open document through a
  OpenWand-owned, random user-local named pipe persisted in LibreOffice's own
  startup configuration. LibreOffice can therefore be opened before or after
  OpenWand once the integration is installed, and the action does not take keyboard
  focus.
- OpenWand verifies the requested result and the unchanged foreground window.
...
- Chart creation never writes the source range. In particular, OpenWand does not use
...
- A real hidden-workbook smoke test created `OpenWandChart` from `A1:C7`, verified
...
- A second live acceptance started LibreOffice from the normal user profile
  without `--accept`, connected through the persisted OpenWand named pipe, and then
...
- A formula-preservation acceptance captured `A1:B3` through UNO, created
  `OpenWandChart`, kept focus unchanged, and verified that `=1+1` and `=2+2`
...
- An already-running LibreOffice process cannot load a newly installed startup
  ...
  whether Calc or OpenWand starts first.
...
real, hidden disposable Excel instance. OpenWand created a table and chart, read back
...
- OpenWand recognizes VS Code, VS Code Insiders, Cursor, and Windsurf windows without
...
- An empty saved UTF-8 file is also supported as an exact whole-file insertion;
  this lets OpenWand create the first contents of a new file without editor focus.
- An Untitled buffer uses the text target captured when the General caller opens. OpenWand
...
- An off-screen acceptance smoke used OpenWand's real `FlowController.intent_chosen`
  route and the configured ChatGPT model. The model proposed the fix, OpenWand
...
- The user then saved the tab as `C:\Users\sunny\Desktop\test openwand vs code.txt`.
  OpenWand resolved the exact live window and Desktop path, ran the configured model,
...
- OpenWand recognizes explicit form-fill requests in Chrome/Edge/Brave/Chromium.
- A OpenWand-managed Chromium DevTools endpoint snapshots visible editable text,
...
- OpenWand validates every field and select option, renders a sanitized HTML table
...
Chrome and the real configured OpenWand model. The model produced three assignments,
...
adopted with OpenWand's private DevTools endpoint. Automatic managed-session launch
...
discovery requires a per-session OpenWand marker so it cannot adopt an unrelated
...
2. Add OpenWand-owned Google and Microsoft account OAuth/token storage, concrete
...
| 5 | Add Calc action planning, HTML preview, and Apply to the General caller | Complete for one column-chart action in OpenWand-managed Windows Calc sessions |
...
`ListObjects` (tables), and `ChartObjects`. OpenWand should orchestrate these Excel
...
- 2026-08-01: A normal running LibreOffice instance cannot be given a new UNO
  listener after startup. OpenWand reads the recorded Calc selection off-screen
...
- 2026-08-01: Calc's accessible Chart command creates the requested chart but
  ...
  marked `WS_EX_NOACTIVATE`. It is not a production action transport. OpenWand may
  keep using UI Automation for bounded context capture, but chart mutation must
  use LibreOffice's built-in UNO API.
- 2026-08-01: The first safe Calc executor connects only to a OpenWand-managed UNO
...
- 2026-08-03: LibreOffice's persistent `ooSetupConnectionURL` was validated in a
  ...
  OpenWand now provisions a random named-pipe endpoint once and waits through slow
  LibreOffice startup with a truthful four-second progress update. This removes
  the recurring OpenWand-before-Calc launch-order restriction. Only a LibreOffice
...
- 2026-08-01: The focusless executor passed a real hidden-workbook smoke test:
  it created `OpenWandChart` from `A1:C7`, verified the chart and source fingerprint,
...
- 2026-08-01: The first VS Code off-screen OpenWand/model smoke completed the real
...
- 2026-08-01: A live empty untitled-tab test reached the real model-generated
  ...
  saved-file adapter cannot own an unsaved buffer; it did not invalidate OpenWand's
  separately captured editor paste-back target.
- 2026-08-01: The follow-up live saved-file acceptance succeeded on the user's
  Desktop file. OpenWand fixed a basename-only VS Code path-resolution gap by using
...
- 2026-08-01: Action latency becomes structured private telemetry rather than a
  ...
  under OpenWand's user-data logs and exclude user content and full paths.
...
- 2026-08-01: The first corrected streaming/topmost trace measured 11.513 seconds
  ...
  that forced tool call. OpenWand now emits an immediate truthful local action-stage
...
- 2026-08-01: Untitled VS Code tabs now use the target OpenWand captures before the
  intent overlay takes focus. Windows retains collapsed UI Automation caret
  ranges as well as selected ranges; macOS retains its focused AX element. OpenWand
...
- 2026-08-01: The original Windows anchored paste still activated VS Code while
  ...
  is therefore never treated as action success: OpenWand verifies the resulting
  ...
  a OpenWand-managed Chromium DevTools input session; ordinary SendInput cannot be
  both target-specific and non-interfering.
- 2026-08-01: Chromium DevTools input was proven only while the isolated VS Code
  page retained session focus. In the complete OpenWand/model flow, both DevTools
...
- 2026-08-01: The supported VS Code Extension API bridge passed the complete
  isolated smoke: real OpenWand model -> sanitized HTML diff preview -> approval ->
  ...
  physical cursor, keyboard, or saved-file mutation. OpenWand owns and authenticates
  this small local bridge; the user is not asked to find or manually install an
  extension. Shipping still requires automatic bridge loading for OpenWand-managed
...
- 2026-08-03: Managed Chrome became the next real application slice. On a
  ...
  through a private DevTools endpoint; the real OpenWand model mapped the request to
  exact field IDs, OpenWand rendered and auto-approved the normal preview in the
