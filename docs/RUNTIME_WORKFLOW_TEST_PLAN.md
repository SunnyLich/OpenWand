...
- **472 / 472** have completed dependency audits; no function remains marked pending.
- **197 / 197** declared A -> B interaction matrices are accepted. Every one of the 472 functions is now an endpoint in at least one audited matrix. These include launcher, Settings, appearance, platform, local-file, onboarding, provider/model routing, conversation and context behavior, speech, memory, privacy, add-on/MCP, multi-agent, worker lifecycle, desktop integration, shortcut search/runtime registration, composer-key behavior, chat zoom, profile mutation, optional-installer outcomes, setup-check recovery, and Reset All persistence/credential cleanup.
...
- The master runner now removes its completed named basetemps and collects abandoned `pytest_<pid>_*` trees only after confirming that their owner process is dead. Live concurrent runs and the explicit `OPENWAND_KEEP_PYTEST_TEMP` debugging mode are preserved.
...
- Uninstall acceptance now builds both packaged and source plans with production safety validation, verifies every exact confirmation target, and executes the native self-removing helpers only against isolated temporary OpenWand app/data/model trees.
...
- Conversation-engine acceptance now saves all six OpenWand/ChatGPT/Claude x OpenWand/agent owner states through the real Settings UI. The OpenWand/agent combination is correctly unavailable; all five valid states immediately drive the production brain handler, proving OpenWand full-history handoff versus resumable agent sessions and isolated OpenWand, Codex-developer, and Claude-system prompts.
...
- Add-on acceptance now clicks the real archive/folder install, enable, Settings, Logs, and dependency-environment controls, pairing each UI route with real isolated installs, persistence, host processes, and repair state. Enabled/disabled and declared/missing permission matrices cover query/response hooks, tools, intent rows, hotkeys, tray actions, notifications, settings fields, and capped private auxiliary LLM calls. The MCP bridge now loads a real `servers.json`, handshakes with a child JSON-RPC server, discovers and executes its tools, and cleanly stops it; the external OpenWand context server advertises and returns all five desktop context operations.
- Multi-agent acceptance now triggers both real tray actions; submits and copies a complete visible task spec; edits scope/globs/context/models/fallbacks/completion/parallel settings; adds, customizes, removes, pairs, and resets agents and communications; and drives live approvals, tabs, agent health/messages, card layout, pause/resume, nudge, cancel, diff/folder actions, retry/continue, and historical artifacts. Production runner tests pair those controls with role gates, provider routing, parallel read-only briefings, overlapping disjoint writes, and same-file lease exclusion. The newly visible Copy current app context control excludes OpenWand's process, reuses the shared document/browser extraction and privacy redaction paths, applies a marked 12,000-character budget, preserves manual task context, and carries both into the runner prompt.
...
Build automated workflows that mimic real use of every user-visible OpenWand function and expose runtime problems.
...
- Launch OpenWand, wait for its workers, open Settings, change a shortcut, save, restart, and verify the setting survived.
- Select text in a simulated external application, open an intent, submit it, stream a result, cancel it, and verify OpenWand returns to idle.
- Open chat, attach a file, send a message, switch conversations, restart OpenWand, and reopen the conversation.
- Start an optional installer, interrupt it, restart OpenWand, and verify no broken package state or temporary files remain.

Mocks or fakes are allowed only at external boundaries that CI cannot safely control, such as provider servers, operating-system permissions, microphones, and keychains. The OpenWand code between the user action and that boundary should remain real.
...
- **Cleanup:** Verify no OpenWand-owned temporary files or orphan processes remain.
...
- Open, close, and interact with OpenWand windows.
...
Faults must be injected at the external boundary, not by replacing the OpenWand function being tested.

### 4. State and leak inspector

Record a baseline before each workflow and compare it at teardown:

- OpenWand worker and helper processes.
- Non-daemon threads.
...
- OpenWand-owned pytest and installer temporary directories.
...
- CI reports zero unexpected crashes, exceptions, hangs, corrupt stores, leaked resources, and OpenWand-owned temporary leftovers.
