@echo off
REM OpenWand - double-click to start.
...
set "STAMP_FILE=.venv\.openwand-deps.stamp"
set "REBUILD_VENV=0"

REM CI/diagnostic callers may supply an already-provisioned compatible Python.
if defined OPENWAND_LAUNCH_PYTHON (
  set "VPY=%OPENWAND_LAUNCH_PYTHON%"
  "%OPENWAND_LAUNCH_PYTHON%" "scripts\check_python_version.py" "!WANT!" >nul 2>nul
  if errorlevel 1 (
    echo ERROR: OPENWAND_LAUNCH_PYTHON is not Python !WANT!.
    exit /b 1
  )
  call :runtime_deps_ok
  if errorlevel 1 (
    echo ERROR: OPENWAND_LAUNCH_PYTHON is missing required runtime dependencies.
    exit /b 1
  )
  goto run
)
...
:run
if not defined OPENWAND_REPO_ROOT set "OPENWAND_REPO_ROOT=%CD%"
set "PYTHONUNBUFFERED=1"
"%VPY%" -m runtime.supervisor.app
exit /b %ERRORLEVEL%

:runtime_deps_ok
"%VPY%" -c "import PySide6; import anydoc; import dotenv; import PIL; import numpy; import av.about" >nul 2>nul
exit /b %ERRORLEVEL%
