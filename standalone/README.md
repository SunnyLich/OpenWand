...
This entry point exercises Wisp's scoped Agent Team engine without launching
Qt, the overlay, or any Wisp worker process.

Wisp chat uses this detached contract when its model delegates substantial
...
Run a real task contract with the provider/model configured for Wisp:
...
so approval-gated actions are declined; use Wisp's UI when interactive approval
is required.
