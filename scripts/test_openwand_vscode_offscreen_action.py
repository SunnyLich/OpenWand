"""Exercise OpenWand's VS Code intent route with the configured model and no GUI focus."""
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="openwand-vscode-smoke-") as temp_dir:
            path = Path(temp_dir) / "demo.py"
            original = "def add_one(value):\n    return value\n"
            selected = "def add_one(value):\n    return value"
            path.write_text(original, encoding="utf-8")
            active_app = {
                "name": "demo.py - openwand-smoke - Visual Studio Code",
        ...
        result = {
            "flow": "OpenWand FlowController.intent_chosen",
