...
def inner() -> int:
    ...
    marker = f'print("OpenWand background smoke {time.time_ns()}")'
    result = {"marker": marker, "isolated_desktop": True}
    with tempfile.TemporaryDirectory(prefix="openwand-vscode-background-") as profile:
...
def parent(child_script: Path | None = None, inner_argument: str = "--inner") -> int:
    ...
    desktop_name = f"OpenWandBackgroundTest-{uuid.uuid4().hex}"
