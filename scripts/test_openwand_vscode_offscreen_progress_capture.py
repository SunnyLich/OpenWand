"""Capture a real OpenWand VS Code action timeline without touching the desktop."""
...
class OffscreenCaptureUI(Worker):
    ...
    def save_timeline(self) -> None:
        ...
        lines = ["# Off-screen OpenWand live-progress capture", ""]
...
def main() -> int:
    ...
    test_file = output_dir / "openwand_live_progress_test.py"
    test_file.write_text("def add_one(value): return value\n", encoding="utf-8")
    os.environ["OPENWAND_ACTION_TRACE_PATH"] = str(output_dir / "action-timings.jsonl")
        ...
        try:
            ...
            pending = PendingInvocation(
                ...
                context={
                    "platform": "win32",
                    "active_app": {
                        "name": "openwand_live_progress_test.py - Visual Studio Code",
    ...
    worker = threading.Thread(target=run_flow, name="openwand-offscreen-flow", daemon=True)
    worker.start()
    pump = QTimer()
