"""Real Wisp model -> preview -> managed Chrome form API smoke."""
...
from test_wisp_vscode_background_desktop import parent, stage, terminate_profile_processes  # noqa: E402
from test_wisp_vscode_cdp_flow_real import Worker  # noqa: E402
from test_wisp_vscode_cdp_real import Cdp, reserve_port  # noqa: E402
...
def inner() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="wisp-chrome-form-flow-") as profile:
        ...
        try:
            ...
            last_error = ""
            active_app = {
                "name": "Wisp Browser Action Test - Google Chrome",
            ...
            flow.intent_chosen(
                "Fill this form with full name Sunny Wisp, email sunny@example.test, and country tw."
            )
            ...
            result.update(
                {
                    ...
                    "preview_has_values": bool(
                        previews
                        and "Sunny Wisp" in str(previews[-1]["params"].get("html") or "")
                    ...
                    "values_verified": (
                        values.get("#full-name") == "Sunny Wisp"
