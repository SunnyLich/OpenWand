"""Visible no-focus demo of OpenWand editing a file shared with a separate app.
...
the separate process, mirrors the resulting file in a OpenWand-owned workspace, and
...
TARGET_TEXT = (
    "OpenWand changed this shared file. Your mouse and keyboard stayed yours."
)
...
def _run_external_app(
    ...
    app = QApplication(["openwand-presence-external-app"])
    app.setStyle("Fusion")
    window = QWidget(None)
    ...
    editor = QLineEdit("Waiting for OpenWand to update the shared file...", content)
    ...
    hint = QLabel(
        "This is another process. OpenWand talks to its standard Windows accessibility interface.",
        content,
    )
    ...
    def save_text(value: str) -> None:
        ...
        saved.setText("Saved just now · visible to OpenWand")
...
def _run_parent(output_directory: Path) -> int:
    ...
    class WorkspaceWindow(QWidget):
        def __init__(self, shared_file: Path) -> None:
            ...
            self.setWindowTitle("OpenWand Shared Workspace")
    ...
    screenshot_file = output_directory / "openwand-shared-presence-demo.png"
    evidence_file = output_directory / "evidence.json"

    app = QApplication(["openwand-shared-presence-demo"])
    app.setStyle("Fusion")
    ...
    with tempfile.TemporaryDirectory(prefix="openwand-presence-demo-") as temp_dir:
        ...
        try:
            ...
            receipt = driver.execute(
                SemanticOperation(
                    ...
                    preconditions=(
                        StateCondition(
                            StateField.VALUE,
                            "Waiting for OpenWand to update the shared file...",
                        ),
                    ),
                )
            )
            ...
            presence.show_text_caret(marker_bounds, "OpenWand agent")
            app.processEvents()
            focus_checkpoints.append(_foreground_window())
