"""Real Wisp model -> preview -> official VS Code Extension API smoke."""
...
from test_wisp_vscode_cdp_flow_real import Worker  # noqa: E402
...
def inner() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="wisp-vscode-extension-flow-") as profile:
        launch_env = os.environ.copy()
        launch_env["WISP_VSCODE_BRIDGE_PORT"] = str(bridge_port)
        launch_env["WISP_VSCODE_BRIDGE_TOKEN"] = token
        ...
        try:
            ...
            flow.intent_chosen(
                "Create a tiny Python function named wisp_api_test that returns the string 'api works'."
            )
            ...
            result.update(
                {
                    ...
                    "preview_has_diff": bool(
                        previews and "wisp_api_test" in str(previews[-1]["params"].get("html") or "")
                    ),
                    "api_apply_called": bool(applies),
                    "api_apply_response": applies[-1].get("result") if applies else {},
                    "logical_text": logical_text,
                    "text_verified": "wisp_api_test" in logical_text and "api works" in logical_text,
                }
            )
            stage(
                "flow_verified",
