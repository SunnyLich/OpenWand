"""Real disposable VS Code Untitled insertion smoke using OpenWand's captured range."""
...
def main() -> int:
    ...
    marker = f'print("OpenWand Untitled real smoke {time.time_ns()}")'
    result: dict[str, Any] = {
        "original_foreground": original_foreground,
        "marker": marker,
    }

    with tempfile.TemporaryDirectory(prefix="openwand-vscode-untitled-") as temp_dir:
        ...
        try:
            ...
            if not token:
                raise RuntimeError("OpenWand did not capture the real collapsed Monaco caret")
