"""Exercise semantic UIA editing in a OpenWand-owned off-screen form without OS input."""
...
def _child(handshake: Path) -> int:
    ...
    app = QApplication(["openwand-offscreen-interaction-smoke"])
    form = QWidget(None)
    form.setWindowTitle("OpenWand semantic interaction smoke")
    form.setWindowFlags(
        Qt.WindowType.Tool
    ...
    display_name.setObjectName("openwandSmokeDisplayName")
    display_name.setAccessibleName("Display name")
    hints = QCheckBox("Enable hints", form)
    hints.setObjectName("openwandSmokeEnableHints")
...
def _parent() -> int:
    ...
    hwnd = 0
    with tempfile.TemporaryDirectory(prefix="openwand-interaction-smoke-") as temp_dir:
