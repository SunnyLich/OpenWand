"""Real-app demo: Wisp updates a shared file and VS Code mirrors it live.

VS Code runs with extensions disabled and a disposable profile on an isolated
Windows desktop. Wisp writes the shared file directly; no mouse, keyboard, or
editor injection is used. The final screenshot combines Wisp's live file view
...
from test_wisp_vscode_background_desktop import (  # noqa: E402
...
from test_wisp_vscode_cdp_real import Cdp, reserve_port, wait_for_target  # noqa: E402

OUTPUT_ENV = "WISP_REAL_VSCODE_DEMO_OUTPUT"
INITIAL_TEXT = 'def greet(name):\n    return f"Hello, {name}"\n'
UPDATED_TEXT = (
    'def greet(name):\n'
    '    return f"Hello, {name} — updated by Wisp"\n\n'
    'print(greet("Sunny"))\n'
)
...
def _render_workspace(shared_file: Path, destination: Path) -> None:
    """Render the actual Wisp-owned file surface without an activity checklist."""
    ...
    app = QApplication.instance() or QApplication(["wisp-real-vscode-workspace"])
    app.setStyle("Fusion")
    window = QWidget()
    ...
    file_name = QLabel("▰  wisp-real-demo.py", files)
    ...
    editor_heading = QLabel("WISP-REAL-DEMO.PY  ·  LIVE CONTENT", editor_area)
    ...
    if not window.grab().save(str(destination), "PNG"):
        raise RuntimeError("Wisp's shared workspace surface could not be captured.")
    window.close()
    app.processEvents()
...
def _compose_screenshot(workspace_path: Path, vscode_path: Path, destination: Path) -> None:
    ...
    app = QApplication.instance() or QApplication(["wisp-real-vscode-compositor"])
    ...
    painter.drawText(label_rect, Qt.AlignmentFlag.AlignCenter, "Wisp agent")
...
def _inner() -> int:
    ...
    shared_file = output_dir / "wisp-real-demo.py"
    vscode_capture = output_dir / "vscode-live.png"
    workspace_capture = output_dir / "wisp-workspace-live.png"
    final_capture = output_dir / "wisp-real-vscode-demo.png"
    ...
    with tempfile.TemporaryDirectory(prefix="wisp-real-vscode-profile-") as profile_value:
        ...
        try:
            ...
            while "wisp-real-demo.py" not in title.casefold() and time.monotonic() < deadline:
                time.sleep(0.2)
            ...
            while time.monotonic() < deadline:
                ...
                if "updated by Wisp" in rendered and 'greet("Sunny")' in rendered:
                    break
                time.sleep(0.2)
            editor_updated = "updated by Wisp" in rendered and 'greet("Sunny")' in rendered
            ...
            if not saved:
                ...
                # identical reviewed content through Wisp's shared-file boundary.
                shared_file.write_text(UPDATED_TEXT, encoding="utf-8")
                saved = shared_file.read_text(encoding="utf-8") == UPDATED_TEXT
                save_method = "Wisp shared-file persistence fallback"
            if not saved:
            ...
            file_tab = client.evaluate(
                ...
                (() => {
                  ...
                  const tab = tabs.find(node => /wisp-real-demo\\.py/i.test(
